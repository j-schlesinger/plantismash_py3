...
try:
    ...
    libImport = True
except ImportError as ImportWarning:
    libImport = False
try:
...
def write(seq_records, options):
    ...
    if 'extrarecord' in options and seq_record.id in options.extrarecord:
        if "MetabolicModelDataObj" in options.extrarecord[seq_record.id].extradata:
            target_model = pickle.loads(
                options.extrarecord[seq_record.id].extradata["MetabolicModelDataObj"]
            )
            # Output files
            ...
            write_cobra_model_to_sbml_file(
                ...
                + os.sep
                + "antiSMASH_model_with_template_%s.xml" % (options.modeling),
            )
            ...
            fp1 = open(
                options.metabolicmodeldir
                + os.sep
                + "antiSMASH_model_with_template_%s_reactions.txt" % options.modeling,
                "w",
            )
            fp2 = open(
                options.metabolicmodeldir
                + os.sep
                + "antiSMASH_model_with_template_%s_metabolites.txt" % options.modeling,
                "w",
            )
            fp1.write(
                "Reaction ID"
            ...
            # Output on screen
            logging.debug("Number of genes in final model: %s",
                          len(target_model.genes))
            logging.debug(
                "Number of reactions in final model: %s", len(
                    target_model.reactions)
            )
            logging.debug(
                "Number of metabolites in final model: %s",
                len(target_model.metabolites),
            )
            ...
            for i in range(len(target_model.metabolites)):
                ...
                print(
                    "%s\t%s\t%s\t%s"
                    % (metab.id, metab.name, metab.formula, metab.compartment),
                    file=fp2,
                )
            ...
            logging.warning(
                "options.extrarecord[%s].extradata does not have a key 'MetabolicModelObjData'\navailale keys: %s"
                % (
                    seq_record.id,
                    ", ".join(
                        list(
                            options.extrarecord[seq_record.id].extradata.keys())
                    ),
                )
            )
