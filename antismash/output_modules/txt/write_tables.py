...
def write_genome(txt, info, options):
    ...
    geneclusters = [
        "%s_c%s" % (info.seq_record.id.partition(".")[0], nr) for nr in info.clusternrs
    ]
    txt.write("\t".join(["genome accession", "geneclusters"]) + "\n")
    txt.write(
        info.seq_record.id.partition(".")[0] + "\t" + ";".join(geneclusters) + "\n"
    )
...
def write_BGC(txt, info, options):
    ...
    for BGCnr in info.clusternrs:
        ...
        detection_rules_used = (
            '"' + ";".join(get_detection_rules(cluster_feature)) + '"'
        )
        BGC_range = ";".join(
            [str(cluster_feature.location.start), str(cluster_feature.location.end)]
        )
        ...
        NRPSs_PKSs = ";".join(
            [
                ...
                and len(
                    [
                        qual
                        for qual in cds.qualifiers["sec_met"]
...
def write_signature_gene_info(txt, info, options):
    ...
    txt.write(
        "\t".join(
            ["signature gene", "pHMM hits", "e-value", "bit score", "number of seeds"]
        )
        + "\n"
    )
    for BGCnr in info.clusternrs:
        ...
        for cds in signature_genes:
            ...
            for domain in domains:
                ...
                txt.write(
                    "\t".join([gene_ID, domain_name, evalue, bitscore, nr_seeds]) + "\n"
                )
...
def write_NRPS_PKS(txt, info, options):
    ...
    for BGCnr in info.clusternrs:
        ...
        cluster_id = "{seq_id}_c{cluster_nr}".format(
            seq_id=info.seq_record.id, cluster_nr=BGCnr
        )
        NRPSs_PKSs = [
            cds
            for cds in cluster_gene_features
        ...
        for cds in NRPSs_PKSs:
            ...
            aSDomains = [
                dom
                for dom in utils.get_cluster_aSDomain_features(
                    cluster_feature, info.seq_record
                )
                if utils.features_overlap(cds, dom)
                and utils.get_gene_id(cds)
                in [dom.qualifiers["locus_tag"], dom.qualifiers["locus_tag"][0]]
            ]
...
def write_RiPP(txt, info, options):
    ...
    for BGCnr in info.clusternrs:
        ...
        for RiPP in RiPP_features:
            ...
            annotation = [
                qual.partition("predicted class: ")[2]
            ...
            core_peptide = [
                ...
                for qual in note_quals
                if "predicted core seq:" in qual
            ][0]
            mol_weight = [
                qual.partition("molecular weight: ")[2]
                for qual in note_quals
                if "molecular weight: " in qual
            ][0]
            monoiso_mass = [
                qual.partition("monoisotopic mass: ")[2]
                for qual in note_quals
            ...
            if "alternative weights" in note_quals:
                ...
                alt_mol_weights = ""
            nr_bridges = [
                qual.partition("number of bridges: ")[2]
                for qual in note_quals
