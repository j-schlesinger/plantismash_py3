        ...
        if record.id in options.extrarecord:
            if "extradata" in options.extrarecord[record.id]:
                if "orig_id" in options.extrarecord[record.id].extradata:
                    js_rec["orig_id"] = options.extrarecord[record.id].extradata[
                        "orig_id"
                    ]
...
def convert_clusters(record, annotations, options):
    ...
    for cluster in utils.get_cluster_features(record):
        ...
        js_cluster["orfs"] = convert_cds_features(
            record, features, annotations, options
        )
        ...
        if "knownclusterblast" in cluster.qualifiers:
            ...
            bestcluster = [
                kcluster for kcluster in knownclusters if kcluster.startswith("1.")
            ]
            if not len(bestcluster) == 1:
                ...
                logging.debug(
                    'Found closest cluster "%s" for cluster no. %s'
                    % (js_cluster["knowncluster"], utils.get_cluster_number(cluster))
                )
...
def convert_cds_features(record, features, annotations, options):
    ...
    for feature in features:
        ...
        if (prefix + js_orf["locus_tag"]) in options.hmm_results:
            prefix = "%s:" % record.id.replace(":", "_")
            for hsp in sorted(
                options.hmm_results[prefix + js_orf["locus_tag"]],
    ...
    if "sec_met" in feature.qualifiers:
        for ann in feature.qualifiers["sec_met"]:
            if ann.startswith("Percentage identity"):
                pct_identities = "<br>".join(
                    [
                        (pci.split("=")[0] + " (" + pci.split("=")[1] + " &#37;)")
                        for pci in ann[len("Percentage identity: ") :].split(",")
                        if len(pci.split("=")) == 2
                    ]
                )
                if len(pct_identities) > 0:
                    template += (
                        '<span class="bold">&#37; identity with other core genes:</span><br>'
                        + pct_identities
                        + "<br><br>\n"
                    )
...
%(smcog_tree_line)s<br>"""
    ...
    if type_ == "transport":
        url = (
            "http://blast.jcvi.org/er-blast/index.cgi?project=transporter;"
            "program=blastp;database=pub/transporter.pep;"
            "sequence=sequence%%0A%s" % sequence
        )
        transport_blast_line = (
            '<a href="%s" target="_new">TransportDB BLAST on this gene<br>' % url
        )
...
def get_ASF_predictions(feature):
    ...
    ASFsec_met_quals = [
        sec_met_qual[16:]
        for sec_met_qual in feature.qualifiers.get("sec_met", [""])
        if sec_met_qual.startswith("ASF-prediction")
    ]
...
def load_cog_annotations():
    ...
    type_keys = {"B": "biosynthetic", "T": "transport", "R": "regulatory", "O": "other"}
    annotations = {}
