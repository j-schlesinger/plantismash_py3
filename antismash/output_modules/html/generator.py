...
def add_no_result_note(d, options):
    ...
    if options.input_type == "nucl":
        ...
        td.text(
            "No secondary metabolite biosynthesis proteins were found in the input sequence(s)"
        )
    tr.append(td)
...
def add_overview_entry(d, cluster, odd):
    ...
    for subtype in subtypes:
        a = pq("<a>")
        a.attr(
            "href", "http://antismash.secondarymetabolites.org/help#{}".format(subtype)
        )
        if config.taxon == "plants":
            a.attr(
                "href",
                "http://plantismash.secondarymetabolites.org/help#{}".format(subtype),
            )
    ...
    if cluster["BGCid"] != "-":
        ...
        a.attr(
            "href",
            "http://mibig.secondarymetabolites.org/repository/{}/index.html".format(
                cluster["BGCid"].split("_")[0]
            ),
        )
        a.attr("target", "_blank")
...
def add_cluster_page(d, cluster, seq_record, options, extra_data, seq_id):
    ...
    if options.input_type == "nucl":
        text = (
            seq_record.name
            + " - Gene Cluster %(idx)s. Type = %(type)s. Location: %(start)s - %(end)s nt. "
        )
    else:
    ...
    if options.input_type == "nucl":
        ...
        legend_text.append(
            generate_legend_entry("legend-type-biosynthetic", "biosynthetic genes")
        )
        legend_text.append(
            generate_legend_entry("legend-type-transport", "transport-related genes")
        )
        legend_text.append(
            generate_legend_entry("legend-type-regulatory", "regulatory genes")
        )
    ...
    details = None
    for handler in handlers:
        details = handler.generate_details_div(
            cluster, seq_record, options, extra_data["js_domains"], details
        )
    ...
    if options.clusterblast:
        ...
        if len(top_ten_clusters) == 0:
            ...
            opt = pq("<option>")
            opt.attr(
                "value", path.join("svg", "clusterblast%s_all.svg" % cluster["idx"])
            )
            opt.text("All hits")
    ...
    if options.subclusterblast:
        ...
        opt = pq("<option>")
        opt.attr(
            "value", path.join("svg", "subclusterblast%s_all.svg" % cluster["idx"])
        )
        opt.text("All hits")
        cb_select.append(opt)
        subclusters_added = 0
        ...
        if (
            path.exists(
                path.join(
                    ...
                    "svg",
                    "subclusterblast%s_all.svg" % cluster["idx"],
                )
            )
            and subclusters_added > 0
        ):
            cb_download = pq("<button>")
    ...
    if options.knownclusterblast:
        ...
        opt = pq("<option>")
        opt.attr(
            "value", path.join("svg", "knownclusterblast%s_all.svg" % cluster["idx"])
        )
        opt.text("All hits")
        cb_select.append(opt)
        ...
        if (
            path.exists(
                path.join(
                    options.outputfoldername,
                    "svg",
                    "knownclusterblast%s_all.svg" % cluster["idx"],
                )
            )
            and knownclusters_added > 0
        ):
            cb_download = pq("<button>")
...
def write_geneclusters_js(records, output_dir, extra_data):
    with open(path.join(output_dir, "geneclusters.js"), "w") as h:
        ...
        if config.coexpress:
            h.write(
                "var geo_dataset_info = %s;\n"
                % json.dumps(
                    [geo["info"] for geo in config.geo_dataset], separators=(",", ":")
                )
            )
...
def generate_webpage(seq_records, options):
    ...
    for i in range(len(records)):
        ...
        for cluster in records[i]["clusters"]:
            add_cluster(
                d, cluster, seq_records[i], options, extra_data, odd, seq_records[0].id
            )
            records_written += 1
            odd = not odd
    ...
    with open(path.join(options.outputfoldername, "index.html"), "w") as h:
        ...
        if "run_info" in options:
            h.write(
                "<!-- antiSMASH ver.{ver}, start time {start}, end time {end} -->\n".format(
                    ver=options.run_info["ver"].replace(">", "*"),
                    start=options.run_info["start"].replace(">", "*"),
                    end=options.run_info["end"].replace(">", "*"),
                )
            )
            h.write(
                "<!-- Parameters : {param} -->\n".format(
                    param=options.run_info["param"].replace(">", "*")
                )
            )
        h.write(d.outerHtml())
...
def get_detection_rules(cluster_rec):
    ...
    for note in cluster_rec.qualifiers["note"]:
        if note.startswith("Detection rule(s)"):
            rules.extend(
                [rule.strip().replace("&", "&amp;") for rule in note[41:].split(";")]
            )
...
def load_searchgtr_search_form_template():
    ...
    searchgtrformtemplate = open(
        path.join(utils.get_full_path(__file__, ""), "searchgtr_form.html"), "r"
    )
            ...
            if gene_id in smcogdict:
                ...
                if smcog in gtrcoglist:
                    if not os.path.exists(
                        options.full_outputfolder_path + os.sep + "html"
                    ):
                    ...
                    link_loc = (
                        "html" + os.sep + utils.get_gene_id(feature) + "_searchgtr.html"
                    )
                    options.searchgtr_links[seq_record.id + "_" + gene_id] = link_loc
                    formfile = open(formfileloc, "w")
                    specificformtemplate = searchgtrformtemplateparts[0].replace(
                        "GlycTr", gene_id
                    )
