...
def run_automodel(seq_records, options):
    ...
    #    logging.error(
    #        "The modeling pipeline is only compatible wit COBRApy version 0.2.1; your insstalled version is %s",
    #        cobra.__version__,
    #    )
    #    return False
    ...
    logging.debug(
        "[metabolicmodel] set up model dir as %s and output dir as %s", root, temp_fasta
    )

    model = pickle.load(
        open(root + os.sep + options.modeling + os.sep + "model.p", "rb")
    )
    tempModel_biggRxnid_locusTag_dict = pickle.load(
        open(
            root
            + os.sep
            + options.modeling
            + os.sep
            + "tempModel_biggRxnid_locusTag_dict.p",
            "rb",
        )
    )
    tempModel_exrxnid_flux_dict = pickle.load(
        open(
            root + os.sep + options.modeling + os.sep + "tempModel_exrxnid_flux_dict.p",
            "rb",
        )
    )
    ...
    logging.debug(
        "loading pickle files of the parsed template model and its relevant genbank data.."
    )
    bigg_mnxr_dict = pickle.load(
        open(root + os.sep + "bigg_mnxr_dict.p", "rb"))
    kegg_mnxr_dict = pickle.load(
        open(root + os.sep + "kegg_mnxr_dict.p", "rb"))
    mnxr_kegg_dict = pickle.load(
        open(root + os.sep + "mnxr_kegg_dict.p", "rb"))
    mnxr_rxn_dict = pickle.load(open(root + os.sep + "mnxr_rxn_dict.p", "rb"))
    bigg_mnxm_compound_dict = pickle.load(
        open(root + os.sep + "bigg_mnxm_compound_dict.p", "rb"),
        encoding='bytes'
    )
    mnxm_bigg_compound_dict = pickle.load(
        open(root + os.sep + "mnxm_bigg_compound_dict.p", "rb"),
        encoding='bytes'
    )
    kegg_mnxm_compound_dict = pickle.load(
        open(root + os.sep + "kegg_mnxm_compound_dict.p", "rb"),
        encoding='bytes'
    )
    mnxm_kegg_compound_dict = pickle.load(
        open(root + os.sep + "mnxm_kegg_compound_dict.p", "rb"),
        encoding='bytes'
    )
    mnxm_compoundInfo_dict = pickle.load(
        open(root + os.sep + "mnxm_compoundInfo_dict.p", "rb"),
        encoding='bytes'
    )
    ###################################################################
    ...
    (
        ...
        target_fasta,
    ) = get_targetGenomeInfo(seq_records, options)

    if len(targetGenome_locusTag_ec_dict) == 0:
        logging.error(
            "Error: no EC_number in sequence record; skipping modeling")
        return False
    ...
    run_blastp(
        target_fasta=options.metabolicmodeldir
        + os.sep
        + "targetGenome_locusTag_aaSeq.fa",
        blastp_result=options.metabolicmodeldir
        + os.sep
        + "blastp_targetGenome_against_tempGenome.txt",
        db_dir=root + os.sep + options.modeling + os.sep + "tempBlastDB",
        evalue=1e-30,
    )
    ...
    run_blastp(
        ...
        + options.modeling
        + os.sep
        + "tempModel_locusTag_aaSeq.fa",
        blastp_result=options.metabolicmodeldir
        + os.sep
        + "blastp_tempGenome_against_targetGenome.txt",
    ...
    blastpResults_dict1 = parseBlaspResults(
        ...
        options.metabolicmodeldir
        + os.sep
        + "blastp_targetGenome_against_tempGenome_parsed.txt",
    )
    ...
    blastpResults_dict2 = parseBlaspResults(
        ...
        options.metabolicmodeldir
        + os.sep
        + "blastp_tempGenome_against_targetGenome_parsed.txt",
    )
    ...
    targetBBH_list, temp_target_BBH_dict = getBBH(
        bestHits_dict1, bestHits_dict2)
    ...
    ###################################################################
    logging.debug(
        "labeling reactions with nonhomologous genes to remove from the template model.."
    )
    rxnToRemove_dict = labelRxnToRemove(
        model, temp_target_BBH_dict, tempModel_biggRxnid_locusTag_dict
    )

    logging.debug(
        "removing reactions with nonhomologous genes from the template model.."
    )
    modelPruned, rxnToRemoveEssn_dict, rxnRemoved_dict, rxnRetained_dict = pruneModel(
        model, rxnToRemove_dict, options.automodel.solver
    )
    ...
    ###################################################################
    logging.debug(
        "creating various dictionary files for the nonBBH gene-associted reactions..."
    )

    targetGenome_locusTag_ec_nonBBH_dict = get_targetGenome_locusTag_ec_nonBBH_dict(
        targetGenome_locusTag_ec_dict, nonBBH_list
    )

    rxnid_info_dict, rxnid_locusTag_dict = make_all_rxnInfo_fromRefSeq(
        targetGenome_locusTag_ec_nonBBH_dict, options
    )

    modelPrunedGPR_mnxr_list = get_mnxr_list_from_modelPrunedGPR(
        modelPrunedGPR, bigg_mnxr_dict
    )
    ###################################################################

    ###################################################################
    logging.debug("adding the nonBBH gene-associated reactions...")
    rxnid_to_add_list = check_existing_rxns(
        kegg_mnxr_dict, modelPrunedGPR_mnxr_list, rxnid_info_dict
    )

    mnxr_to_add_list = get_mnxr_using_kegg(rxnid_to_add_list, kegg_mnxr_dict)

    rxnid_mnxm_coeff_dict = extract_rxn_mnxm_coeff(
        mnxr_to_add_list,
        mnxr_rxn_dict,
    ...
    logging.debug(
        "Number of genes in template and pruned models: %s / %s",
        len(model.genes),
        len(modelPruned.genes),
    )
    logging.debug(
        "Number of reactions in template and pruned models: %s / %s",
        len(model.reactions),
        len(modelPruned.reactions),
    )
    logging.debug(
        "Number of metabolites in template and pruned models: %s / %s",
        len(model.metabolites),
        len(modelPruned.metabolites),
    )
