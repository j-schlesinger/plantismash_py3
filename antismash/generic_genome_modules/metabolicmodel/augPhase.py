...
from cobra.io.sbml import (
    # write_sbml_model as write_cobra_model_to_sbml_file,
    # read_sbml_model as create_cobra_model_from_sbml_file,
...
def get_rxnid_from_ECNumber(enzymeEC):
    ...
    rxnid_set = re.findall(
        r"\s+R[0-9]+[0-9]+[0-9]+[0-9]+[0-9]" + "\\b", ecinfo_text)
    rxnid_list = []
...
def get_targetGenome_locusTag_ec_nonBBH_dict(
    ...
    for locusTag in nonBBH_list:
        if locusTag in list(targetGenome_locusTag_ec_dict.keys()):
            targetGenome_locusTag_ec_nonBBH_dict[
                locusTag
            ] = targetGenome_locusTag_ec_dict[locusTag]
...
def make_all_rxnInfo_fromRefSeq(targetGenome_locusTag_ec_nonBBH_dict, options):
    ...
    logging.debug(
        "Querying KEGG to get reaction ID's for %s gene products..., this step can take a while",
    ...
    try:
        ...
        logging.warning(
            "could not read KEGG EC-rxn cache file %s: %s", KEGG_EC_rxnCache, e
        )
    except IOError as e:
    ...
    try:
        ...
        logging.warning(
            "could not read KEGG rxn-Info cache file %s: %s", KEGG_rxnInfoCache, e
        )
    except IOError as e:
        ...
        for enzymeEC in targetGenome_locusTag_ec_nonBBH_dict[locusTag]:
            ...
            if "-" not in enzymeEC:
                if enzymeEC in ECrxnMapping:
                    ...
                    if len(rxnid_list) == 0:
                        logging.debug(
                            "ECrxnmapping for %s resulted in empty list", enzymeEC
                        )
                ...
                for rxnid in rxnid_list:
                    if rxnid in rxnid_info_dictCache:
                        ...
                        logging.debug(
                            "getting reaction information for %s from local cache",
                            rxnid,
                        )
                    else:
                        logging.debug(
                            "querying KEGG to get reaction information for %s", rxnid
                        )
                        ...
                        if rxnid_info_dict[rxnid]:
                            ...
                            logging.warn(
                                "Skipping rxnid %s for adding to cache as it equals None",
                                rxnid,
                            )
...
def check_existing_rxns(kegg_mnxr_dict, modelPrunedGPR_mnxr_list, rxnid_info_dict):
    ...
    for rxnid in list(rxnid_info_dict.keys()):
        ...
        if rxnid in list(kegg_mnxr_dict.keys()):
            ...
            if (
                kegg_mnxr not in modelPrunedGPR_mnxr_list
...
def get_correct_metab_coeff(
    converted_metab_id, metab_coeff, metab_type, mnxm_coeff_dict, mnxm_metab_list
):
    ...
    if converted_metab_id in mnxm_metab_list:
        ...
        mnxm_coeff_dict[converted_metab_id] = (
            overlap_metab_coeff + float(metab_coeff) * -1
        )
    else:
        ...
        if "(" not in substrates and "(" not in products:
            ...
            for substrate in substrates:
                ...
                if substrate[1] in list(mnxm_bigg_compound_dict.keys()):
                    mnxm_coeff_dict = get_correct_metab_coeff(
                        mnxm_bigg_compound_dict[substrate[1]],
                        substrate[0],
                        metab_type,
                        mnxm_coeff_dict,
                        mnxm_subs_list,
                    )
                    mnxm_subs_list.append(
                        mnxm_bigg_compound_dict[substrate[1]])

                elif substrate[1] in list(mnxm_kegg_compound_dict.keys()):
                    mnxm_coeff_dict = get_correct_metab_coeff(
                        mnxm_kegg_compound_dict[substrate[1]],
                        substrate[0],
                        metab_type,
                        mnxm_coeff_dict,
                        mnxm_subs_list,
            ...
            for product in products:
                ...
                if product[1] in list(mnxm_bigg_compound_dict.keys()):
                    mnxm_coeff_dict = get_correct_metab_coeff(
                        mnxm_bigg_compound_dict[product[1]],
                        product[0],
                        metab_type,
                        mnxm_coeff_dict,
                        mnxm_prod_list,
                    ...
                    mnxm_coeff_dict = get_correct_metab_coeff(
                        mnxm_kegg_compound_dict[product[1]],
                        product[0],
                        metab_type,
                        mnxm_coeff_dict,
                        mnxm_prod_list,
            ...
            # e.g., ATP + ADP <=> ADP + ATP
            overlap_check = check_overlap_subs_prod(
                mnxm_subs_list, mnxm_prod_list)
        ...
        if rxnid_info_dict[rxnid] != None:
            ...
            for metab in rxnid_mnxm_coeff_dict[rxnid]:
                ...
                if metab_compt in modelPrunedGPR.metabolites:
                    rxn.add_metabolites(
                        {
                            modelPrunedGPR.metabolites.get_by_id(
                                metab_compt
...
def check_exrxn_flux_direction(tempModel_exrxnid_flux_dict, target_exrxnid_flux_dict):
    ...
    for exrxn_id in list(tempModel_exrxnid_flux_dict.keys()):
        if exrxn_id in list(target_exrxnid_flux_dict.keys()):
            ...
            ratio_exrxn_flux = float(target_exrxn_flux) / \
                float(template_exrxn_flux)

            # Similar species are allowed to uptake nutrients within a decent range
            if (
                float(target_exrxn_flux) * float(template_exrxn_flux) > 0.0
                and 0.2 < ratio_exrxn_flux
                and ratio_exrxn_flux < 2.0
            ):
