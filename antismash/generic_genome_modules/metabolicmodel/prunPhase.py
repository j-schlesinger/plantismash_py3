...
from cobra.flux_analysis import single_gene_deletion as single_deletion
...
def get_targetGenomeInfo(seq_records, options):
    targetFastaFilename = (
        options.metabolicmodeldir + os.sep + "targetGenome_locusTag_aaSeq.fa"
    )
    ...
    for seq_record in seq_records:
        ...
        for feature in utils.get_cds_features(seq_record):
            ...
            logging.debug(
                "Found locus_tag %s for feature %s",
                locusTag,
...
def make_blastDB(query_fasta, options):
    ...
    if (
        os.path.isfile(options.metabolicmodeldir + os.sep + "targetBlastDB.psq")
        == False
    ):
...
def parseBlaspResults(inputFile, outputFile):
    ...
    for line in fp:
        ...
        blastpResults_dict[key] = {
            "query_locusTag": qseqid,
            "db_locusTag": sseqid,
            "evalue": evalue,
            "score": score,
            "length": length,
        ...
        print(
            "%s\t%s\t%s\t%f\t%d\t%f" % (qseqid, sseqid, evalue, score, length, pident),
            file=fp2,
        )
        itemnum += 1
    fp.close()
                ...
                for target_locusTag2 in dic2[temp_locusTag]:
                    ...
                    if target_locusTag == target_locusTag2:
                        ...
                        if temp_locusTag not in list(temp_target_BBH_dict.keys()):
                            ...
                            temp_target_BBH_dict[temp_locusTag].append(
                                (target_locusTag)
                            )
...
def labelRxnToRemove(model, temp_target_BBH_dict, tempModel_biggRxnid_locusTag_dict):
    ...
    for biggRxnid in list(tempModel_biggRxnid_locusTag_dict.keys()):
        ...
        if (
            "Transport" not in rxn.name
            and "transport" not in rxn.name
            and "Exchange" not in rxn.name
            and "exchange" not in rxn.name
        ):
            booleanList = makeBooleanFormat(
                temp_target_BBH_dict, tempModel_biggRxnid_locusTag_dict[biggRxnid]
            )
    ...
    for rxnid in list(rxnToRemove_dict.keys()):
        ...
        if rxnToRemove_dict[rxnid] == False:
            growth_rate_dict, solution_status_dict, problem_dict = single_deletion(
                model, list([rxnid]), element_type="reaction", solver=solver_arg
            )
            ...
            if str(list(solution_status_dict.values())[0]) == "optimal":
                ...
                if float(list(growth_rate_dict.values())[0]) >= 0.01:
                    ...
                    logging.debug(
                        "Removed reaction: %s, %s, %s, %s",
                        rxnid,
                        list(growth_rate_dict.values())[0],
                        len(model.reactions),
