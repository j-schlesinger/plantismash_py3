...
class FakeFeature(object):
    ...
    def __repr__(self):
        return "FakeFeature(%r, %r, %r)" % (self.location, self.type, self.qualifiers)
...
class TestUtils(unittest2.TestCase):
    def setUp(self):
        ...
        for t in self.types:
            ...
            f.qualifiers["note"] = [
                "PFAM-Id: PF%05d" % idx,
                "smCOG: SMCOG{0:04d}:FAKE{0:04d} (Score: {0} ; E-value: 1.1e-1{0};".format(
                    idx
                ),
            ]
    ...
    def test_get_all_features_of_type_with_query(self):
        ...
        f = utils.get_all_features_of_type_with_query(
            self.rec, "aSDomain", "note", "PFAM-Id: PF00007"
        )
        self.assertEqual(self.rec.features[6], f[0])

        f = utils.get_all_features_of_type_with_query(
            self.rec, "aSDomain", "note", "PFAM-Id: PF00008"
        )
    ...
    def test_get_feature_dict(self):
        ...
        ids = [
            f.qualifiers["locus_tag"][0] for f in self.rec.features if f.type == "CDS"
        ]
        keys = list(fd.keys())
        ids.sort()
        keys.sort()
...
class TestContainmentHelpers(unittest2.TestCase):
    def setUp(self):
        self.features = [
            FakeFeature(
                "cluster", FeatureLocation(
                    25, 50), {"note": ["Cluster number: 1"]}
            ),
            ...
            FakeFeature(
                "cluster", FeatureLocation(4711, 4800), {
    ...
    def test_get_cluster_type(self):
        ...
        cluster = FakeFeature("cluster", FeatureLocation(
            23, 42), {"product": ["fake"]})
    ...
    def test_get_cluster_features_of_type(self):
        ...
        self.assertEqual(
            [], utils.get_cluster_features_of_type(self.record, "pks"))
        self.assertEqual(
            [self.features[0]], utils.get_cluster_features_of_type(
                self.record, "nrps")
        )
        self.assertEqual(
            [self.features[-1]],
    ...
    def test_features_overlap(self):
        ...
        self.assertFalse(utils.features_overlap(
            self.features[0], self.features[1]))
        self.assertFalse(utils.features_overlap(
            self.features[1], self.features[0]))

        self.assertTrue(utils.features_overlap(
            self.features[0], self.features[3]))
        self.assertTrue(utils.features_overlap(
            self.features[3], self.features[0]))

        self.assertTrue(utils.features_overlap(
            self.features[0], self.features[4]))
        self.assertTrue(utils.features_overlap(
            self.features[4], self.features[0]))

        self.assertTrue(utils.features_overlap(
            self.features[0], self.features[5]))
        self.assertTrue(utils.features_overlap(
            self.features[5], self.features[0]))

    def check_sorting(self):
        for i in range(len(self.features) - 1):
            if self.features[i].location.start > self.features[i + 1].location.start:
                return False
            if (
                self.features[i].location.start == self.features[i +
                                                                 1].location.start
                and self.features[i].location.end > self.features[i + 1].location.end
            ):
                return False
    ...
    def test_cmp_feature_location(self):
        ...
        self.assertEqual(
            0, utils.cmp_feature_location(self.features[0], self.features[0])
        )
        self.assertEqual(
            -1, utils.cmp_feature_location(self.features[1], self.features[0])
        )
        self.assertEqual(
            1, utils.cmp_feature_location(self.features[0], self.features[1])
        )
...
class TestExecutableHelpers(unittest2.TestCase):
    def setUp(self):
        ...
        proc.communicate = Mock(
            "proc.communicate", returns=("output", "error"), tracker=self.tt
        )
    ...
    def test_locate_executable(self):
        ...
        self.assertEqual(utils.locate_executable(
