...
class aSDB(object):
    def __init__(self, options):
        ...
        if "BioSQLconfig" not in options:
            logging.exception(
                "Parameters for database access not defined in default.cfg. Bailing out..."
            )
            sys.exit(1)  # FIXME: Library functions should never call sys.exit()
        try:
            self.server = BioSeqDatabase.open_database(
                driver=options.BioSQLconfig.dbdriver,
            ...
            logging.exception(
                "Error setting up the database object; Check parameters in default.cfg"
            )
            raise e
        self.db = {}
    ...
    def connect(self, namespace):
        ...
        if namespace not in self.db:
            try:
                ...
                logging.exception(
                    "Could not assign BioSQL namespace/database %s, probably namespace does not exist",
                    namespace,
                )
                raise e
    ...
    def delete(self, entryid):
        ...
        try:
            ...
            logging.warn(
                "Could not delete record %s from database/namespace %s",
                entryid,
                self.namespace,
            )
            raise e
    ...
    def fetch_entryid_by_name(self, name):
        ...
        try:
            namespace_id = self.db[self.namespace].adaptor.fetch_dbid_by_dbname(
                self.namespace
            )
            dbid = self.db[self.namespace].adaptor.fetch_seqid_by_display_id(
                dbid=namespace_id, name=name
            )
        except Exception as e:
    ...
    def generate_namespace(self, namespace, description):
        try:
            self.db[namespace] = self.server.new_database(
                db_name=namespace, description=description
            )
            self.namespace = namespace
            # self.commit()
            logging.debug(
                "namespace %s generated for storing genome seq_records", namespace
            )
        except Exception as e:
    ...
    def switch_namespace(self, new_namespace):
        if new_namespace not in self.db:
            logging.exception(
                "cannot switch to namespace %s; namespace in not connected",
                new_namespace,
            )
            sys.exit(1)
