...
class extradata(object):
    def __init__(self, options):
        ...
        try:
            dbconn = psycopg2.connect(
                database=options.BioSQLconfig.dbdb,
    ...
    def check_SeqID(self, genbank_id, secondary_id):
        ...
        SQL = """SELECT
                 ...
                 LIMIT 1;""".format(
            schema=self.extraschema, table=self.extradataTablename
        )
    ...
    def delete(self, genbank_id, secondary_id):
        ...
        SQL = """DELETE FROM
                 ...
                 WHERE
                    {schema}.{table}.genbank_id = %s AND
                    {schema}.{table}.secondary_id LIKE %s;""".format(
            schema=self.extraschema, table=self.extradataTablename
        )
        ...
        try:
            ...
            logging.warning(
                "Could not delete record with genbank_id, secondary_id %s, %s from database\n"
                "psycopg error is: %s\n",
                genbank_id,
                secondary_id,
                e,
        ...
        if not db_cur.rowcount == 1:
            logging.debug(
                "psycopg2 delete operation returned %s deleted rows", db_cur.rowcount
            )
        self.commit()
    ...
    def insert(self, genbank_id, secondary_id, string):
        ...
        try:
            ...
            logging.warning(
                "Could not insert record with genbank_id %s\npsycopg error is: %s\n",
                genbank_id,
                e,
            )
    ...
    def update(self, genbank_id, secondary_id, string):
        ...
        SQL = """UPDATE
                ...
                WHERE
                    {schema}.{table}.genbank_id = %s AND
                    {schema}.{table}.secondary_id = %s;""".format(
            schema=self.extraschema, table=self.extradataTablename
        )
        try:
            db_cur.execute(
    ...
    def fetchOneRow(self, genbank_id, secondary_id):
        ...
        try:
            ...
            logging.warning(
                "Could not fetch row with genbank_id, secondary_id %s, %s from database\n"
                "psycopg error is: %s\n",
                genbank_id,
                secondary_id,
                e,
        ...
        if not db_cur.rowcount == 1:
            logging.warning(
                "psycopg2 SELECT operation returned %s rows", db_cur.rowcount
            )
    ...
    def getExtradataHash(self, genbank_id):
        ...
        SQL = """SELECT
                 ...
                 FROM
                    {schema}.{table}
                 WHERE
                    {schema}.{table}.genbank_id = %s ;""".format(
            schema=self.extraschema, table=self.extradataTablename
        )
        ...
        try:
            ...
            logging.warning(
                "Could not get ExtradataHash record for genbank_id, %s from database\n"
                "psycopg error is: %s\n",
                genbank_id,
                e,
            )
