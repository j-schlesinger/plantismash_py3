...
def generate_details_div(cluster, seq_record, options, js_domains, details=None):
    ...
    cluster = utils.get_cluster_by_nr(
        seq_record, cluster["idx"]
    )  # use seqrecord.feature
    details = pq("<div>")
    ...
    if len(result_list) > 0:
        ...
        for result in result_list:
            if result.cds_id:
                ...
                id_list.append(
                    "Region with unknown ID from %s to %s"
                    % (result.position[0], result.position[1])
                )

        sidepanel.html(
            "%s Coding sequences with repeats found:<br> %s"
