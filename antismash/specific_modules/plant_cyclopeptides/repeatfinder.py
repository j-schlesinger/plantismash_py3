    ...
    for feat in seq_record.features:
        if feat.type == "CDS":
            ...
            if len(feat.qualifiers["top_kmer_hits"]) > 0:
                ...
                if (
                    len(feat.qualifiers["table"][0]) > 3
                ):  # TODO replace this with assesment module as bool check
...
def scan_seqfeature_translation(seq_record, feature, k=3):
    ...
    if feature.qualifiers["translation"]:
        seq = feature.qualifiers["translation"][
            0
        ]  # feature.qualifiers[key] gives a list of a single string, this gives the translation
    else:
...
def build_table(feature):
    table = [feature.qualifiers["top_kmer_word"]] * len(
        feature.qualifiers["top_kmer_hits"]
    )
...
def run_fbk_per_cluster(
    filepath_list, outpath_list, output_type="txt", teiresias=False
):
...
def main():
    mass_run(
        "C:/users/ezmod/data/results",
        "C:/users/ezmod/data/algorithm_output_v6",
        output_type="genbank",
    )
