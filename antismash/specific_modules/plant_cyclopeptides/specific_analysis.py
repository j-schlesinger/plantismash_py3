...
def specific_analysis(seq_record, options):
    ...
    for cluster in clusters:
        if (
            ...
            or "cyclopeptide" not in cluster.qualifiers["product"][0]
        ):
            continue
...
def fbk_output_to_result(seq_record):
    ...
    for feat in seq_record.features:
        ...
        if "pattern" in feat.qualifiers:
            if (
                len(feat.qualifiers["table"][0]) > 5
                and len(feat.qualifiers["top_kmer_hits"]) >= 3
            ):
                has_ripp = True
                result.pattern = feat.qualifiers["pattern"]
                result.instances = feat.qualifiers[
                    "table"
                ]  # top_kmer_hits for locations
        if (
            "ripp_evidence" in feat.qualifiers
            and len(feat.qualifiers["ripp_evidence"]) > 0
        ):
            has_ripp = True
...
class Result:
    ...
    def decode(self, qualifier):
        ...
        self.cds_id = q[
            5
        ]  # TODO removed length if statement. if it breaks this is probably the reason
        keys = q[6].split("|")
