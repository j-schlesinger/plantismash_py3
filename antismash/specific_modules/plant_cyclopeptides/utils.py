    ...
    for feature in features:
        ...
        if "sec_met" in feature.qualifiers:
            ...
            for domain in domains:
                ...
                domainlist.append(
                    [hit_id, int(domstart), int(domend), evalue, float(bitscore)]
                )
            if len(domainlist) > 0:
...
def execute(commands, input=None):
    ...
    try:
        ...
        logging.debug(
            "%r %r returned %r", commands, input[:40] if input is not None else None, e
        )
        raise
        ...
        if "geo" in feature.qualifiers:
            ...
            for go in geo_array:
                ...
                if gene_id not in result[go["rec_id"]]["metadata"]["nodes"]:
                    result[go["rec_id"]]["metadata"]["nodes"][gene_id] = [gene_type]
                samples_order = []
