...
def write_result_summary(result, instance_line_nr=5):
    ...
    if len(result.instances) > 1:
        ...
        summary += (
            "Repeat occurs %s times in a sequence of %s amino acids"
            % (len(result.instances), len(result.sequence))
            + br
        )
        summary += "Location between %s and %s %s" % (
            result.position[0],
            result.position[1],
            br,
        )
        coverage = round(
            (
                float(len(result.instances) * len(result.instances[0]))
                / len(result.sequence)
            )
            * 100,
            2,
        ...
        summary += "Instances:<br> %s%s" % (
            format_instances(result.instances, instance_line_nr),
            br,
        )
            ...
            if len(result.evidence[ek]) > 1:
                ...
                if not summary_text_printed:
                    if repeat_found:
                        ...
                        summary += (
                            "The following known motifs were found in CDS %s <br>"
                            % (result.cds_id)
                        )
                        summary += "Location between %s and %s %s" % (
                            result.position[0],
                        ...
                        summary += (
                            "The following known motifs were found between locations %s and %s <br>"
                            % (result.position[0], result.position[1], br)
                        )

                    summary_text_printed = True

                summary += "%s was found %s times in this sequence<br>" % (
                    ek,
                    len(result.evidence[ek]),
                )
                seqprint = True
    if seqprint:
        html_seq = ""
        if repeat_found:
            ...
            if known_motif_found:
                ...
                html_seq = generate_html_seq(
                    result.sequence, result.pattern, known_motifs
                )
            else:
...
def write_feature_summary(feature, instance_line_nr=5):
    ...
    summary = ""
    summary += hr
    summary += "Repeat found between %s and %s %s" % (
        feature.location.start,
        feature.location.end,
        br,
    )
    summary += (
        "Repeat was found %s times in a sequence of %s amino acids"
        % (len(feature.qualifiers["table"]), len(feature.qualifiers["translation"]))
        + br
    )
    coverage = round(
        (
            float(
                len(feature.qualifiers["table"]) * len(feature.qualifiers["table"][0])
            )
    ...
    summary += "Coverage of %s %s" % (coverage, "%") + br
    summary += "Instances:<br> %s%s" % (
        format_instances(feature.qualifiers["table"], instance_line_nr),
        br,
    )
...
def write_evidence_summary(feat):
    ...
    for key in feat.qualifiers["ripp_evidence"]:
        ...
        if current_evidence_len > 0:
            ...
            summary += "the pattern %s, associated with %s was found %s times %s" % (
                pattern,
                known_ripp_str,
                current_evidence_len,
                br,
            )
...
def create_output(feature):
    ...
    highlight_seq(
        ...
        instances=feature.qualifiers["table"],
        output_key="html_seq",
    )
    highlight_seq(
        feature.qualifiers["translation"][0],
        instances=feature.qualifiers["glutamine_aromat_structures"][0],
        output_key="html_seq_glutamine",
    )
...
def add_html_output(seqrecord):
    seqrecord.annotations[
        "html_output"
    ] = "Cluster %s from %s with %s potential repeat regions" % (
        seqrecord.id,
        seqrecord.description,
        len(seqrecord.annotations["repeat_regions"]),
    )
    ...
    for feat in seqrecord.features:
        if "has_repeat" in feat.qualifiers:
            ...
            if (
                len(feat.qualifiers["table"]) > 0
                or len(feat.qualifiers["ripp_evidence"]) > 0
            ):
                create_output(feat)
