...
class Lantipeptide(object):
    ...
    def __repr__(self):
        return "Lantipeptide(%s..%s, %s, %r, %r, %s, %s(%s))" % (
            self.start,
            self.end,
            self.score,
            self.lantype,
...
def predict_cleavage_site(query_hmmfile, target_sequence):
    ...
    """
    hmmer_res = utils.run_hmmpfam2(query_hmmfile, bytes(target_sequence, "utf-8"))
    resvec = None
    for res in hmmer_res:
        for hits in res:
            ...
            for hsp in hits:
                resvec = Lantipeptide(
                    hsp.query_start - 1, hsp.query_end, hsp.bitscore, lanti_type
                )
                return resvec
    return resvec
...
def predict_class_from_gene_cluster(seq_record, cluster):
    ...
    for feature in utils.get_cds_features(seq_record):
        if (
            ...
            or feature.location.end > cluster.location.end
        ):
            continue
...
def run_lantipred(seq_record, query, lant_class):
    ...
    if thresh_dict[lant_class] > result.score:
        logging.debug(
            "%r: Score %0.2f below threshold %0.2f for class %r"
            % (
...
def result_vec_to_features(orig_feature, res_vec):
    ...
    leader_feature.qualifiers["note"].append(
        "predicted leader seq: %s" % res_vec.leader
    )
    ...
    core_feature.qualifiers["note"].append(
        "monoisotopic mass: %0.1f" % res_vec.monoisotopic_mass
    )
    core_feature.qualifiers["note"].append(
        "molecular weight: %0.1f" % res_vec.molecular_weight
    )
    if res_vec.alternative_weights:
        weights = ["%0.1f" % x for x in res_vec.alternative_weights]
        core_feature.qualifiers["note"].append(
            "alternative weights: %s" % "; ".join(weights)
        )
    core_feature.qualifiers["note"].append(
        "number of bridges: %s" % res_vec.number_of_lan_bridges
    )
...
def specific_analysis(seq_record, options):
    ...
    for cluster in clusters:
        if (
            "product" not in cluster.qualifiers
            or "lantipeptide" not in cluster.qualifiers["product"][0]
        ):
            continue
