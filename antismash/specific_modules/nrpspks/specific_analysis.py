...
from .substrates import (
    run_nrps_substr_spec_predictions,
    run_pks_substr_spec_predictions,
    parse_substr_spec_predictions,
)
from .parsers import (
    calculate_consensus_prediction,
    generate_domainnamesdict,
    insert_modified_monomers,
)
...
def _map_domaintype(domain_type):
    ...
    if domain_type in domain_type_dict:
...
def create_rawoutput_storage_folder(options):
    ...
    options.raw_predictions_outputfolder = path.abspath(
        path.join(options.outputfoldername, "nrpspks_predictions_txt")
    )
...
def write_data_to_seq_record(pksnrpsvars, seq_record, options):
    ...
    for feature in pksnrpsvars.pksnrpscoregenes:
        ...
        for qualifier in secmetqualifiers:
            if "NRPS/PKS Domain:" not in qualifier:
                ...
                match_pos_obj = re.search(
                    "NRPS/PKS Domain: ([\w-]+) \((\d+)\-(\d+)\)\. E-value: ([\de\.-]+)\. Score: ([\de\.a-]+);",
                    qualifier,
                )
                if not match_pos_obj:
                    logging.exception(
                        "Exception: could not extract domain string from qualifier %s:"
                        % qualifier
                    )
                    sys.exit(1)
                ...
                domainFeature = SeqFeature(
                    loc, type=options.FeatureTags.pksnrpsdomains_tag
                )
                domainFeature.qualifiers["domain"] = [domain_type]
                if "locus_tag" in feature.qualifiers:
                    domainFeature.qualifiers["locus_tag"] = feature.qualifiers[
                        "locus_tag"
                    ]
                else:
                ...
                domainFeature.qualifiers["evalue"] = [
                    str("{:.2E}".format(float(evalue)))
                ]
                ...
                if "transl_table" in feature.qualifiers:
                    ...
                    transl_table = 1
                domainFeature.qualifiers["translation"] = [
                    str(
                        domainFeature.extract(seq_record).seq.translate(
                            table=transl_table
                        )
                    )
                ]
                ...
                if domain_type == "AMP-binding":
                    ...
                    domainFeature.qualifiers["asDomain_id"] = [
                        "nrpspksdomains_" + domainname
                    ]
                    domainFeature_specificity.append(
                        "NRPSpredictor2 SVM: %s"
                        % pksnrpsvars.nrps_svm_preds[domainname]
                    )
                    domainFeature_specificity.append(
                        "Stachelhaus code: %s" % pksnrpsvars.nrps_code_preds[domainname]
                    )
                    domainFeature_specificity.append(
                        "Minowa: %s" % pksnrpsvars.minowa_nrps_preds[domainname]
                    )
                    domainFeature_specificity.append(
                        "consensus: %s" % pksnrpsvars.consensuspreds[domainname]
                    )

                    newqualifier = (
                        qualifier
                        + " NRPS/PKS Domain: %s; Substrate specificity predictions: %s (NRPSPredictor2 SVM), %s (Stachelhaus code), %s (Minowa), %s (consensus);"
                        % (
                            domainname,
                            pksnrpsvars.nrps_svm_preds[domainname],
                            pksnrpsvars.nrps_code_preds[domainname],
                            pksnrpsvars.minowa_nrps_preds[domainname],
                            pksnrpsvars.consensuspreds[domainname],
                        )
                    )
                    ...
                    newqualifier_detailed = (
                        ...
                        % (
                            domainname,
                            pksnrpsvars.nrps_code_preds_details[domainname],
                            pksnrpsvars.nrps_svm_preds_details[domainname],
                            pksnrpsvars.minowa_nrps_preds_details[domainname],
                            pksnrpsvars.consensuspreds[domainname],
                        )
                    )
                    ...
                    domainFeature_specificity.append(
                        "PKS signature: %s" % pksnrpsvars.pks_code_preds[domainname]
                    )
                    domainFeature_specificity.append(
                        "Minowa: %s" % pksnrpsvars.minowa_pks_preds[domainname]
                    )
                    ...
                    if "transatpks" not in cluster_info["product"][0]:
                        ...
                        newqualifier = (
                            qualifier
                            + " Substrate specificity predictions: %s (PKS signature), %s (Minowa), %s (consensus);"
                            % (
                                pksnrpsvars.pks_code_preds[domainname],
                                pksnrpsvars.minowa_pks_preds[domainname],
                                pksnrpsvars.consensuspreds[domainname],
                            )
                        )
                        # BiosynML: appending substrate prediction data into 'newqualifier_detailed'
                        newqualifier_detailed = (
                            ...
                            + " Substrate specificity predictions: %s (PKS signature), %s (Minowa), %s (consensus);"
                            % (
                                pksnrpsvars.pks_code_preds_details[domainname],
                                pksnrpsvars.minowa_pks_preds_details[domainname],
                                pksnrpsvars.consensuspreds[domainname],
                            )
                        )
                        updated_secmetqualifiers.append(newqualifier)
                        updated_secmetqualifiers_predictions.append(
                            newqualifier_detailed
                        )
                    # For transatpks
                        ...
                        domainFeature_specificity.append(
                            "consensus: %s"
                            % pksnrpsvars.consensuspreds_transat[domainname]
                        )
                        newqualifier = (
                            qualifier
                            + " Substrate specificity predictions: %s (PKS signature), %s (Minowa), %s (consensus);"
                    ...
                    domainFeature_specificity.append(
                        "Minowa: %s" % pksnrpsvars.minowa_cal_preds[domainname]
                    )
                    newqualifier = (
                        qualifier
                        + " Substrate specificity predictions: %s (Minowa);"
                        % (pksnrpsvars.minowa_cal_preds[domainname])
                    )
                    # BiosynML: appending substrate prediction data into 'newqualifier_detailed'
                    newqualifier_detailed = (
                        qualifier
                        + " Substrate specificity predictions: %s (Minowa);"
                        % (pksnrpsvars.minowa_cal_preds_details[domainname])
                    )
                    updated_secmetqualifiers.append(newqualifier)
                    updated_secmetqualifiers_predictions.append(newqualifier_detailed)
                    ...
                    domainFeature_specificity.append(
                        "KR activity: %s" % pksnrpsvars.kr_activity_preds[domainname]
                    )
                    domainFeature_specificity.append(
                        "KR stereochemistry: %s"
                        % pksnrpsvars.kr_stereo_preds[domainname]
                    )
                    newqualifier = (
                        qualifier
                        + " Predicted KR activity: %s; Predicted KR stereochemistry: %s;"
                        % (
                            pksnrpsvars.kr_activity_preds[domainname],
                            pksnrpsvars.kr_stereo_preds[domainname],
                        )
                    )
                    # BiosynML: appending substrate prediction data into 'newqualifier_detailed'
        ...
        if gene_id in pksnrpsvars.consensuspred_gene_dict:
            feature.qualifiers[options.QualifierTags.product_prediction] = "-".join(
                pksnrpsvars.consensuspred_gene_dict[gene_id]
            )
    ...
    for cluster in clusters:
        ...
        if clusternr in pksnrpsvars.compound_pred_dict:
            structpred = pksnrpsvars.compound_pred_dict[clusternr]
            cluster.qualifiers["note"].append("Monomers prediction: " + structpred)
            cluster.qualifiers["note"].append(
                "Structure image: structures/genecluster%s.png" % clusternr
            )
