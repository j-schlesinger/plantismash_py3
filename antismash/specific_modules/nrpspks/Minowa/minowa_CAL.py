...
def run_minowa_cal(infile2, outfile):
    ...
    for i in namesb:
        ...
        hmmresults = hmmsearch(
            "hmm_infile.fasta", path.join(cal_hmms_dir, "Acetyl-CoA.hmm")
        )
        hmmname = "Acetyl-CoA"
        ...
        hmm_scores.append(hmmscore[0])
        hmmresults = hmmsearch(
            "hmm_infile.fasta", path.join(cal_hmms_dir, "fatty_acid.hmm")
        )
        hmmname = "fatty_acid"
        hmm_names.append(hmmname)
        hmmscore = hmmscores(hmmresults)
        ...
        hmm_scores.append(hmmscore[0])
        hmmresults = hmmsearch(
            "hmm_infile.fasta", path.join(cal_hmms_dir, "shikimic_acid.hmm")
        )
