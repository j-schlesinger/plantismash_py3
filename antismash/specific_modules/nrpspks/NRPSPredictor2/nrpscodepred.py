...
def run_nrpscodepred(options):
    ...
    for k in querysignames:
        ...
        for i in signaturenames:
            ...
            positions = list(range(len(querysigseq)))
            score = 0
            for j in positions:
        ...
        for hit in allsortedhits:
            aa = hit.split("__")[-1]
            score = scoredict[hit]
            if aa not in nextbesthitsdict and aa != "xxx":
        ...
        outfile2.write(
            ";".join(["%s(%s)" % (aa, nextbesthitsdict[aa]) for aa in nextbesthits])
            + "\n"
        )
    outfile1.close()
    outfile2.close()
