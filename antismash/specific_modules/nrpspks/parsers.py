...
def parse_nrps_preds(options, pksnrpsvars):
    ...
    if len(pksnrpsvars.nrpsnames) > 0:
        ...
        for i in parts:
            ...
            if tophit in list(substratetransdict2.keys()):
                tophit = substratetransdict2[tophit]
            pksnrpsvars.minowa_nrps_preds[acc] = tophit.lower()
            pksnrpsvars.minowa_nrps_preds_details[acc] = (
                "<b>Minowa HMM method A-domain<br>Substrate specificity prediction top hits:</b><br>\n"
                + partlines[1]
                + "<br>\n"
                + partlines[2]
                + "<br>\n"
        ...
        nrpspredictorfile1 = open(
            options.raw_predictions_outputfolder
            + os.sep
            + "ctg"
            + str(options.record_idx)
            + "_nrpspredictor2_svm.txt",
            "r",
        )
        nrpspredictorfile2 = open(
            options.raw_predictions_outputfolder
            + os.sep
            + "ctg"
            + str(options.record_idx)
            + "_nrpspredictor2_codes.txt",
            "r",
        )
        ...
        for k in lines:
            ...
            pksnrpsvars.nrps_svm_preds_details[tabs[0]] = (
                "<b> NRPSPredictor2 SVM prediction details:</b><br>\n8 Angstrom 34 AA code:<br>\n"
                + tabs[1]
        ...
        for k in lines:
            ...
            pksnrpsvars.nrps_code_preds_details[tabs[0]] = (
                "<b> NRPSPredictor2 Stachelhaus code prediction:</b><br>\n"
                + tabs[1]
                + "<br><br>\n\n"
            )
...
def parse_pks_preds(options, pksnrpsvars):
    ...
    if len(pksnrpsvars.pksnames) > 0:
        ...
        for i in parts:
            partlines = i.split("\n")
            acc = partlines[0]
            if partlines[2].split("\t")[0] in substratetransdict:
                tophit = substratetransdict[partlines[2].split("\t")[0]]
            else:
                tophit = "pk"
            pksnrpsvars.minowa_pks_preds[acc] = tophit
            pksnrpsvars.minowa_pks_preds_details[acc] = (
                "<b>Minowa HMM method AT-domain<br>Substrate specificity prediction top hits:</b><br>\n"
                + partlines[1]
                + "<br>\n"
                + partlines[2]
                + "<br>\n"
        ...
        for i in parts:
            ...
            if len(partlines) > 2:
                ...
                if len(prots) >= 3:
                    pksnrpsvars.pks_code_preds_details[acc] = (
                        "<b>PKS Active Site Signature method<br>AT-domain substrate specificity prediction top hits:</b><br>\nCode:"
                        + partlines[0].split("\t")[1]
                        + "<br>\n"
                        + codes[0]
                        + " - "
                ...
                pksnrpsvars.pks_code_preds_details[
                    acc
                ] = "<b>PKS Active Site Signature method<br>No AT-domain substrate specificity prediction hits above 40% identity.<br>\n\n"
    if len(pksnrpsvars.calnames) > 0:
        minowa_cal_file = open(
            options.raw_predictions_outputfolder
            + os.sep
            + "ctg"
            + str(options.record_idx)
        ...
        for i in parts:
            ...
            pksnrpsvars.minowa_cal_preds_details[acc] = (
                "<b>Minowa HMM method<br>CAL-domain substrate specificity prediction top hits:</b><br>\n"
                + partlines[1]
                + "<br>\n"
                + partlines[2]
                + "<br>\n"
        ...
        for k in j:
            if "transatpks" not in cluster_info["product"][0]:
                if k[0] == "PKS_AT":
                    ...
                    preds = []
                    preds.append(
                        pksnrpsvars.minowa_pks_preds[locus + "_AT" + str(nrat)]
                    )
            ...
            if k[0] == "CAL_domain":
                nrcal += 1
                if (
                    pksnrpsvars.minowa_cal_preds[locus + "_CAL" + str(nrcal)]
                    in available_smiles_parts
                ):
                    pksnrpsvars.consensuspreds[
...
def insert_modified_monomers(pksnrpsvars, seq_record):
    ...
    for key in list(pksnrpsvars.domainnamesdict.keys()):
            ...
            for at_idx in range(len(at_list)):
                ...
                for kr_idx in range(len(kr_list)):
                    if at_idx + 1 <= len(at_list) - 1:
                        if (
                            kr_list[kr_idx] > at_list[at_idx]
                            and kr_list[kr_idx] < at_list[at_idx + 1]
                        ):
                            if (
                                pksnrpsvars.consensuspreds[
                                    locusTag + "_AT" + str(at_idx + 1)
                                ]
                                == "mal"
                            ):
                                pksnrpsvars.consensuspreds[
                ...
                for dh_idx in range(len(dh_list)):
                    if at_idx + 1 <= len(at_list) - 1:
                        if (
                            dh_list[dh_idx] > at_list[at_idx]
                            and dh_list[dh_idx] < at_list[at_idx + 1]
                        ):
                            if (
                                pksnrpsvars.consensuspreds[
                                    locusTag + "_AT" + str(at_idx + 1)
                ...
                for er_idx in range(len(er_list)):
                    if at_idx + 1 <= len(at_list) - 1:
                        if (
                            er_list[er_idx] > at_list[at_idx]
                            and er_list[er_idx] < at_list[at_idx + 1]
                        ):
                            if (
                                pksnrpsvars.consensuspreds[
                                    locusTag + "_AT" + str(at_idx + 1)
            ...
            for ks_idx in range(len(ks_list)):
                ...
                for kr_idx in range(len(kr_list)):
                    if ks_idx + 1 <= len(ks_list) - 1:
                        if (
                            kr_list[kr_idx] > ks_list[ks_idx]
                            and kr_list[kr_idx] < ks_list[ks_idx + 1]
                        ):
                            if (
                                pksnrpsvars.consensuspreds[
                                    locusTag + "_KS" + str(ks_idx + 1)
                                ]
                                == "mal"
                            ):
                                pksnrpsvars.consensuspreds[
                ...
                for dh_idx in range(len(dh_list)):
                    if ks_idx + 1 <= len(ks_list) - 1:
                        if (
                            dh_list[dh_idx] > ks_list[ks_idx]
                            and dh_list[dh_idx] < ks_list[ks_idx + 1]
                        ):
                            if (
                                pksnrpsvars.consensuspreds[
                                    locusTag + "_KS" + str(ks_idx + 1)
                ...
                for er_idx in range(len(er_list)):
                    if ks_idx + 1 <= len(ks_list) - 1:
                        if (
                            er_list[er_idx] > ks_list[ks_idx]
                            and er_list[er_idx] < ks_list[ks_idx + 1]
                        ):
                            if (
                                pksnrpsvars.consensuspreds[
                                    locusTag + "_KS" + str(ks_idx + 1)
...
def write_substr_spec_preds_to_html(options, pksnrpsvars):
    ...
    for feature in pksnrpsvars.pksnrpscoregenes:
        locus = utils.get_gene_id(feature)
        if (
            "PKS_AT" in pksnrpsvars.domainnamesdict[locus]
            or "AMP-binding" in pksnrpsvars.domainnamesdict[locus]
            or "A-OX" in pksnrpsvars.domainnamesdict[locus]
            or "CAL_domain" in pksnrpsvars.domainnamesdict[locus]
        ):
            j = pksnrpsvars.domaindict[locus]
            nrat = 0
            nra = 0
            ...
            for k in j:
                if k[0] == "PKS_AT":
                    ...
                    htmloutfile = open(
                        options.substrspecsfolder + os.sep + domainname + ".html", "w"
                    )
                    htmloutfile.write(
