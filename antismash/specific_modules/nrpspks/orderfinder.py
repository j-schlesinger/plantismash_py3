    ...
    for genecluster in nrpspksclusters:
        ...
        if len(clusterpksnrpsgenes) > 0:
            (
                ...
                hybridgenes,
                clusterhybridgenes,
            ) = find_cluster_modular_enzymes(clusterpksnrpsgenes, pksnrpsvars)
            ...
            if pksgenes > 3 and pksgenes < 11 and nrpsgenes == 0 and hybridgenes == 0:
                geneorder = perform_docking_domain_analysis(
                    options,
                    clusterpksgenes,
                    utils.get_cluster_number(genecluster),
                    seq_record,
                    pksnrpsvars,
                )
                options.docking[utils.get_cluster_number(genecluster)] = True
            else:
                geneorder = find_colinear_order(
                    clusterpksnrpsgenes, seq_record, pksnrpsvars.domainnamesdict
                )
                options.docking[utils.get_cluster_number(genecluster)] = False
            generate_substrates_order(
                utils.get_cluster_number(genecluster),
                geneorder,
                pksnrpsvars,
                seq_record,
...
def find_clusterpksnrpsgenes(genecluster, pksnrpscoregenes):
    ...
    for gene in pksnrpscoregenes:
        if gene.location.start in range(
            genecluster.location.start, genecluster.location.end
        ) or gene.location.end in range(
            genecluster.location.start, genecluster.location.end
        ):
...
def find_cluster_modular_enzymes(clusterpksnrpsgenes, pksnrpsvars):
    ...
    for j in clusterpksnrpsgenenames:
        ...
        if "PKS" in k and "NRPS" not in k:
            ...
            if (
                "PKS_KS" in pksnrpsvars.domainnamesdict[j]
                or "PKS_AT" in pksnrpsvars.domainnamesdict[j]
            ) and (
                "AMP-binding" not in pksnrpsvars.domainnamesdict[j]
                and "A-OX" not in pksnrpsvars.domainnamesdict[j]
                and "Condensation" not in pksnrpsvars.domainnamesdict[j]
            ):
                pksgenes += 1
                clusterpksgenes.append(j)
            elif (
                "PKS_KS" not in pksnrpsvars.domainnamesdict[j]
                and "PKS_AT" not in pksnrpsvars.domainnamesdict[j]
            ) and (
        ...
        for l in domains:
            if "transatpks" not in cluster_info["product"][0]:
                if "PKS_AT" in l:
                    ...
                    nrat += 1
                    prediction = (
                        prediction + pksnrpsvars.consensuspreds[k + "_AT" + str(nrat)]
                    )
            ...
            if "AMP-binding" in l or "A-OX" in l:
                ...
                nra += 1
                prediction = (
                    prediction + pksnrpsvars.consensuspreds[k + "_A" + str(nra)]
                )
                consensuspred_list.append(
                    pksnrpsvars.consensuspreds[k + "_A" + str(nra)]
                )
                domainnr += 1
            if "CAL_domain" in l:
                if domainnr > 0:
                ...
                nrcal += 1
                prediction = (
                    prediction + pksnrpsvars.consensuspreds[k + "_CAL" + str(nrcal)]
                )
                consensuspred_list.append(
                    pksnrpsvars.consensuspreds[k + "_CAL" + str(nrcal)]
                )
                domainnr += 1
        if k in pksnrpsvars.consensuspred_gene_dict:
            logging.warn(
...
def find_first_and_last_genes(clusterpksgenes, domainnamesdict):
    ...
    for k in clusterpksgenes:
        if "Thioesterase" in domainnamesdict[k] or "TD" in domainnamesdict[k]:
            if endinggene == "":
                ...
                endinggene = ""
        if (
            len(domainnamesdict[k]) >= 2
            and "PKS_AT" == domainnamesdict[k][0]
            and "ACP" == domainnamesdict[k][1]
        ):
            if startergene == "":
                startergene = k
            else:
    ...
    if startergene == "":
        for k in clusterpksgenes:
            if (
                len(domainnamesdict[k]) >= 3
                and "PKS_KS" == domainnamesdict[k][0]
                and "PKS_AT" == domainnamesdict[k][1]
                and "ACP" == domainnamesdict[k][2]
            ):
                if startergene == "":
                    startergene = k
                else:
...
def extractpositions(fasta_file, positions, refsequencename, querysequencename):
    ...
    muscle_names, muscle_seqs = list(zip(*names_seqs))
    return extract_positions_from_refseq(
        muscle_seqs, muscle_names, positions, refsequencename, querysequencename
    )
...
def extract_positions_from_refseq(
    muscle_seqs, muscle_names, positions, refsequencename, querysequencename
):
...
def extract_nterminus(da_dir, clusterpksgenes, seq_record, startergene, feature_by_id):
    ...
    for k in ntermnames:
        ...
        intresidues = extractpositions(
            "muscle.fasta", [2, 15], "EryAIII_5_6_ref", ntermnames[z]
        )
...
def extract_cterminus(da_dir, clusterpksgenes, seq_record, endinggene, feature_by_id):
    ...
    for k in ctermnames:
        ...
        intresidues = extractpositions(
            "muscle.fasta", [55, 64], "EryAII_ref", ctermnames[z]
        )
...
def rank_biosynthetic_orders(
    ntermintresdict, ctermintresdict, startergene, endinggene, possible_orders
):
    ...
    for k in possible_orders:
        ...
        for l in interactions:
            ...
            if (
                (res1a in hydrophobic and res1b in hydrophobic)
                or (res1a in positivecharge and res1b in negativecharge)
                or (res1a in negativecharge and res1b in positivecharge)
            ):
                score += 1
            if (res1a in positivecharge and res1b in positivecharge) or (
                res1a in negativecharge and res1b in negativecharge
            ):
                score = score - 1
            if (
                (res2a in hydrophobic and res2b in hydrophobic)
                or (res2a in positivecharge and res2b in negativecharge)
                or (res2a in negativecharge and res2b in positivecharge)
            ):
                score += 1
            if (res2a in positivecharge and res2b in positivecharge) or (
                res2a in negativecharge and res2b in negativecharge
            ):
                score = score - 1
...
def write_gene_orders_to_html(
    options, geneorders, possible_orders_scoredict, genecluster, startergene, endinggene
):
    ...
    for l in geneorders:
        ...
        if startergene != "" and endinggene != "":
            string = (
                string[:-1]
                + "</td><td>"
                + str(possible_orders_scoredict[tuple(l[1:-1])])
            )
        elif startergene == "" and endinggene != "":
            string = (
                string[:-1]
                + "</td><td>"
                + str(possible_orders_scoredict[tuple(l[:-1])])
            )
        elif startergene != "" and endinggene == "":
            string = (
                string[:-1] + "</td><td>" + str(possible_orders_scoredict[tuple(l[1:])])
            )
        elif startergene == "" and endinggene == "":
            string = (
                string[:-1] + "</td><td>" + str(possible_orders_scoredict[tuple(l)])
            )
...
def perform_docking_domain_analysis(
    options, clusterpksgenes, genecluster, seq_record, pksnrpsvars
):
    ...
    with TemporaryDirectory(change=True):
        ...
        ntermintresdict = extract_nterminus(
            dockinganalysis_dir, clusterpksgenes, seq_record, startergene, feature_by_id
        )
        ctermintresdict = extract_cterminus(
            dockinganalysis_dir, clusterpksgenes, seq_record, endinggene, feature_by_id
        )
...
def find_colinear_order(clusterpksnrpsgenes, seq_record, domainnamesdict):
    ...
    if (
        "Thioesterase" in domainnamesdict[utils.get_gene_id(clusterpksnrpsgenes[0])]
        or "TD" in domainnamesdict[utils.get_gene_id(clusterpksnrpsgenes[0])]
    ):
