...
def generate_sidepanel(cluster, seq_record, options, sidepanel=None):
    ...
    if cluster["idx"] in options.docking and options.docking[cluster["idx"]]:
        warning.text(
            "Rough prediction of core scaffold based on assumed "
    ...
    for feature in features:
        ...
        if len(per_CDS_predictions) > 0:
            ...
            if url:
                ...
                dd.append(a)
                dd.append(" // ")
                url = _get_norine_url_for_specArray(
                    per_CDS_predictions, be_strict=False
                )
                a = pq("<a>")
                a.attr("href", url)
                a.attr("target", "_new")
...
def _get_norine_url_for_cluster(cluster_rec, be_strict=True):
    ...
    for monomers_per_protein in monomers_per_protein_list:
        ...
        if be_strict:
            monomers = [
                _map_as_names_to_norine(element.lower())
                for element in _filter_norine_as(monomers, be_strict=True)
            ]
        else:
            monomers = [
                _map_as_names_to_norine(element.lower()) + "*"
                for element in _filter_norine_as(monomers)
            ]
        # Norine doesn't allow "x*" as a "relaxed" monomer, so we have to replace this with "x"
        monomers = ["x" if x == "x*" else x for x in monomers]

        if len(monomers) > 0:
            nrpslist.append("nrps" + str(i) + "=" + ",".join(monomers))
        i += 1
    urlstring = "http://bioinfo.lifl.fr/norine/fingerPrintSearch.jsp?" + "&".join(
        nrpslist
    )
    ...
    if len(specArray) > 0:
        for domain_specificity_list in specArray:
            if len(domain_specificity_list) == 1:
                if be_strict:
                    modulelist.append(
                        _map_as_names_to_norine(domain_specificity_list[0])
                    )
                else:
                    modulelist.append(
                        _map_as_names_to_norine(domain_specificity_list[0]) + "*"
                ...
                if be_strict:
                    modulelist.append(
                        ...
                        + "|".join(
                            [
                                _map_as_names_to_norine(element)
                                for element in _filter_norine_as(
                                    domain_specificity_list, be_strict=True
                                )
                            ]
                        )
                        + "]"
                ...
                logging.warn(
                    "retrieved emtpy domain list for assembling per protein NORINE link string- this should not happen!!"
                )
        urlstring = (
            "http://bioinfo.lifl.fr/norine/fingerPrintSearch.jsp?nrps1="
            + ",".join(modulelist)
        )
        logging.debug("NORINE URL from array: %s" % urlstring)
        return urlstring
...
def _parse_domain(domain, feature, seq_record):
    ...
    napdoslink = ""
    domainseq = str(utils.get_aa_sequence(feature))[
        int(coordinates[0]) : int(coordinates[-1])
    ]
    if "PKS_KS" in text:
        napdoslink = (
            "http://napdos.ucsd.edu/cgi-bin/process_request.cgi?query_type=aa&amp;ref_seq_file=all_KS_public_12062011.faa&amp;Sequence=%3EKS_domain_from_antiSMASH%0D"
            + domainseq
        )
    elif "Condensation" in text:
        napdoslink = (
            "http://napdos.ucsd.edu/cgi-bin/process_request.cgi?query_type=aa&amp;ref_seq_file=all_C_public_12062011.faa&amp;Sequence=%3EC_domain_from_antiSMASH%0D"
            + domainseq
        )
    blastlink = (
        "http://blast.ncbi.nlm.nih.gov/Blast.cgi?PAGE=Proteins&amp;PROGRAM=blastp&amp;BLAST_PROGRAMS=blastp&amp;QUERY="
        + domainseq
