                ...
                if hmm_mtime > binary_mtime:
                    try:
                        ...
                        failure_messages.append(
                            "Failed to remove outdated binary file for %s: %s"
                            % (hmm, e)
                        )
                        break
                    _, err, retcode = utils.run_hmmpress(hmm)
                    if retcode != 0:
                        failure_messages.append(
                            "Failed to hmmpress %r: %r" % (hmm, err)
                        )
                        import datetime

                        failure_messages.append(
                            "HMM binary files outdated. %s (changed: %s) vs %s (changed: %s)"
                            % (
                                hmm,
                                datetime.datetime.fromtimestamp(hmm_mtime),
                                binary,
                                datetime.datetime.fromtimestamp(binary_mtime),
                            )
                        )
                    break
...
def _update_sec_met_entry(feature, results, clustertype):
    result = "; ".join(
        [res.query_id + " (" + str(res.bitscore) + ")" for res in results]
    )
