...
def filter_nonterminal_docking_domains(seq_record, pksnrpsvars):
    ...
    hitgenes = list(pksnrpsvars.domaindict.keys())
    ...
    for hitgene in hitgenes:
        ...
        for hit in pksnrpsvars.domaindict[hitgene]:
            if hit[0] in dockingdomains:
                if not (
                    hitgenelength - max(hit[1], hit[2]) < 50 or min(hit[1], hit[2]) < 50
                ):
                    to_remove.append(x)
            x += 1
        to_remove.reverse()
...
def run_nrpspks_specific_hmmer(seq_record, withinclustergenes, pksnrpsvars):
    ...
    abmotif_results = utils.run_hmmscan(
        utils.get_full_path(__file__, "abmotifs.hmm"), nrpspksfasta, abmotif_opts
    )
    ...
    pksnrpsvars.domaindict = parse_hmmscan_results(
        nrpspksdomain_results, hmmlengthsdict
    )
    ...
    ksdomain_results = utils.run_hmmscan(
        utils.get_full_path(__file__, "ksdomains.hmm"), nrpspksfasta, nrpspksdomain_opts
    )
...
def name_nrpspks(seq_record, pksnrpsvars, withinclustergenes, options):
    ...
    for feature in withinclustergenes:
        ...
        if k not in pksnrpsvars.domaindict:
            continue
        ...
        eneKSscore = 0
        iterKSscore = 0
        if k in pksnrpsvars.ksdomaindict:
            for i in pksnrpsvars.ksdomaindict[k]:
                ...
                if i[0] == "Iterative-KS":
                    iterKSscore += 1
        if k in pksnrpsvars.domaindict:
            for i in pksnrpsvars.domaindict[k]:
                if (
                    "Cglyc" in domainlist
                    and "Epimerization" in domainlist
                    and "AMP-binding" in domainlist
                    and "PKS_KS" not in domainlist
                    and "PKS_AT" not in domainlist
                ):
                    nrpspkstype = "Glycopeptide NRPS"
                elif (
                    (
                        "Condensation_LCL" in domainlist
                        or "Condensation_DCL" in domainlist
                        or "Condensation_Starter" in domainlist
                    ...
                    or "AMP-binding" in domainlist
                    and ("PKS_KS" in domainlist or "PKS_AT" in domainlist)
                ):
                    ...
                    (
                        ...
                        and "Cglyc" not in domainlist
                        and "Condensation_Dual" not in domainlist
                        and "AMP-binding" not in domainlist
                    )
                    and "PKS_KS" in domainlist
                    and "PKS_AT" not in domainlist
                    and "Trans-AT_docking" in domainlist
                    and traKSscore > modKSscore
                    and traKSscore > iterKSscore
                    and traKSscore > eneKSscore
                ):
                    ...
                    and "PKS_KS" in domainlist
                    and "PKS_AT" in domainlist
                    and iterKSscore > modKSscore
                    and iterKSscore > traKSscore
                    and iterKSscore > eneKSscore
                    and nrKSdomains < 3
                ):
                    ...
                    and "PKS_KS" in domainlist
                    and "PKS_AT" in domainlist
                    and eneKSscore > modKSscore
                    and eneKSscore > traKSscore
                    and eneKSscore > iterKSscore
                    and nrKSdomains < 3
                ):
                    ...
                    and (
                        (
                            modKSscore > eneKSscore
                            and modKSscore > traKSscore
                            and modKSscore > iterKSscore
                        )
                        or nrKSdomains > 3
                    )
                ):
