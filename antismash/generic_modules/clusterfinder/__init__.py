...
def run_cluster_finder(seq_record, options):
    ...
    if len(pfam_features) == 0:
        return 0
    pfam_ids = [
        [xref for xref in feature.qualifiers["db_xref"] if "PFAM: " in xref][
            0
        ].partition("PFAM: ")[2]
    ...
    pfam_features = [
        feature
        for feature in pfam_features
        if len([xref for xref in feature.qualifiers["db_xref"] if "PFAM: " in xref]) > 0
    ]

    options.clusterfinderfolder = (
        options.full_outputfolder_path + os.sep + "clusterfinder"
    )
    ...
    predictions = predict_clusters(
        ...
        start_p,
        graph=True,
        name="ctg_" + str(options.record_idx),
        FilterRepeats=0,
        path=options.clusterfinderfolder,
        outs=pfam_ids,
    )

    if len(pfam_ids) != len(predictions):
        print(
            "Warning: ClusterFinder input length %s != output length %s"
            % (len(pfam_ids), len(predictions))
        )
...
def find_nr_cds(clusterpositions, seq_record):
    ...
    for cds in cdsfeatures:
        if (
            clusterpositions[0] <= int(cds.location.start) <= clusterpositions[1]
            or clusterpositions[0] <= int(cds.location.end) <= clusterpositions[1]
            or int(cds.location.start) <= clusterpositions[0] <= int(cds.location.end)
            or int(cds.location.start) <= clusterpositions[1] <= int(cds.location.end)
        ):
        ...
        if cf_probability >= 0.3:
            ...
            if state == "extend":
                ...
                if is_good_cluster_hit(cdsnr, probabilities, pfam_ids, options):
                    logging.info(
                        "Adding cluster "
                        + str(len(cf_clusters) + 1)
                        + " at position "
                        + str(clusterpositions[0])
                        + " to "
                        + str(clusterpositions[1])
                    )
                    cf_clusters.append(
                        ClusterFinderHit(clusterpositions, mean(probabilities))
                    )
                clusterpositions = []
                pfam_ids = []
...
def annotate_geneclusters(seq_record, options):
    ...
    for cf_cluster in cf_clusters:
        ...
        for cluster in cluster_features:
            cluster_sig_genes = [
                gene
                for gene in utils.get_secmet_cds_features(seq_record)
                if gene in utils.get_cluster_cds_features(cluster, seq_record)
            ]
            if utils.features_overlap(cf_cluster, cluster):
                overlaps = True
                if (
                    options.borderpredict
                ):  # Predict gene cluster borders using ClusterFinder
                    if (
                        (cluster.location.end + cluster.location.start) / 2
                    ) in cf_cluster.location:
                        ...
                        for sig_gene in cluster_sig_genes:
                            ...
                            if cluster.location.start > startpoint:
                                cluster.location = FeatureLocation(
                                    startpoint, cluster.location.end
                                )
                            if cluster.location.end < endpoint:
                                cluster.location = FeatureLocation(
                                    cluster.location.start, endpoint
                                )
                elif (
                    cf_cluster.location.start < cluster.location.start
                    and cf_cluster.location.end > cluster.location.end
                ):
                    cluster.location = cf_cluster.location
                elif cf_cluster.location.start < cluster.location.start:
                    cluster.location = FeatureLocation(
                        cf_cluster.location.start, cluster.location.end
                    )
                elif cf_cluster.location.end > cluster.location.end:
                    cluster.location = FeatureLocation(
                        cluster.location.start, cf_cluster.location.end
                    )
            ...
            for CDS in cf_cluster_CDSs:
                if "sec_met" in CDS.qualifiers:
                    type_sec_met_qualifiers = [
                        feat for feat in CDS.qualifiers["sec_met"] if "Type: " in feat
                    ]
    ...
    clusters = sorted(clusters, key=functools.cmp_to_key(compare_feature_locations))
...
def get_cf_probability(feature):
    ...
    cf_probability_string = [
        note
        for note in feature.qualifiers["note"]
        if "ClusterFinder probability: " in note
    ][0]
...
def is_good_cluster_hit(cdsnr, probabilities, pfam_ids, options):
    ...
    rc = (
        cdsnr >= options.cdsnr
        and mean(probabilities) >= options.cf_prob_thres
        and npfams >= options.cf_npfams
    )
    logging.info(
        "rc = "
        + str(rc)
