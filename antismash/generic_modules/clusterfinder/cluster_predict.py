...
# Read in Files !
out4 = open(PATH_TO_FREQUENCIES + "TP_arr_A.pkl", "rb")
...
A = pickle.load(out4, encoding="bytes")
B = pickle.load(out5, encoding="bytes")
start_p = pickle.load(out6, encoding="bytes")
index_dict = pickle.load(out7, encoding="bytes")
...
def do_HMM(
    ...
    if method == "viterbi":
        ...
        PREDICTED = list(
            (array(viterbi(A, B, start_p, observations, index_dictio)) - 1) * (-1)
        )
        ...
        PREDICTED = list(
            forward_backward(A, B, start_p, observations, index_dict, method="BF")
        )
    ...
    if report == True:
        ...
        if len(outs) == len(PREDICTED):
            ...
            print(
                "OutputWriteError: PREDICTED array length does not match wigh Genome class!",
                file=sys.stderr,
            )
            return None
        output.close()
...
def ploter(Xaxis, Yaxis, LineLength=500000, path=os.getcwd() + "/", name="bla"):
    ...
    for chain in Xaxis:
        ...
        for x in range(len(X)):
            ...
            ax.set_ylim(0.0, 1.0)
            ax.set_xticklabels(
                list(range(x * LineLength / 1000, (x + 1) * LineLength / 1000))[::100]
                + [(x + 1) * LineLength / 1000]
            )
            ax.set_yticklabels(())
            ...
            for tick in ax.yaxis.get_major_ticks():
                ...
                canvas.print_figure(
                    path + os.sep + name + ".cluster_prediction.png", dpi=100
                )
...
def read_input(outs, predictions, filter_repeats=0):
    ...
    for d in outs:
        ...
        if chain not in Info:
            Info[chain] = [[pStart, pStop, PfamID, prob]]
        else:
            Info[chain].append([pStart, pStop, PfamID, prob])

        x += 1

    Info1 = {}
    for chain in list(Info.keys()):
