...
def _make_fake_feature(start, end, probability=None, pfam_id=None, type_=None):
    ...
    if probability is not None:
        feature.qualifiers["note"].append(
            "ClusterFinder probability: %02.4f" % probability
        )
...
class ClusterFinderTest(unittest2.TestCase):
    def setUp(self):
        ...
        mock(
            "antismash.utils.get_cds_features",
            tracker=self.tt,
    ...
    def test_compare_feature_locations(self):
        ...
        self.assertEqual(
            -1, clusterfinder.compare_feature_locations(feature1, feature2)
        )
    ...
    def test_annotate_geneclusters_no_overlaps(self):
        ...
        mock(
            "antismash.utils.get_pfam_features",
            tracker=self.tt,
            returns=self.fake_features,
        )
        ...
        self.assertIn("probability", list(cluster1.qualifiers.keys()))
        self.assertAlmostEqual(0.6429, float(cluster1.qualifiers["probability"][0]))

        self.assertIn("probability", list(cluster2.qualifiers.keys()))
