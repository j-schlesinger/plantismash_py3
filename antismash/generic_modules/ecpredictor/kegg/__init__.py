...
def _getKEGG_speciesLocusTag(CDSFeatureDict):
    ...
    speciesLocusTagDict = {}
    keylist = list(CDSFeatureDict.keys())
    ...
    for chunkIterator in range(0, len(keylist), ChunkSizeMapping):
        ...
        logging.debug(
            "ECpredictor/kegg: doing mapping for CDS %s - %s of %s"
            % (chunkIterator, chunkIterator + ChunkSizeMapping, len(keylist))
        )
            ...
            if "db_xref" in Feature.qualifiers:
                ...
                for db_xref in db_xrefs:
                    ...
                    if "GI:" in db_xref:
                        ...
                        GItoIDDict["ncbi-gi:" + db_xref.split(":")[1]] = keylist[
                            keyindex
                        ]
                        break
                    elif "GeneID:" in db_xref:
                        queryGeneIDList.append(db_xref.split(":")[1])
                        GItoIDDict["ncbi-geneid:" + db_xref.split(":")[1]] = keylist[
                            keyindex
                        ]
        ...
        if len(queryGIList) > 0:
            try:
                ...
                KEGGQuery += urllib.request.urlopen(
                    QueryGI % "+ncbi-gi:".join(queryGIList)
                ).read()
                print()
            except URLError as e:
                logging.warn(
                    "Could not retrieve GI - KEGG results for species:LocusTag query for entries %s - %s \n \
                              Error message %s"
                    % (chunkIterator, chunkIterator + i, e.reason)
                )
        ...
        if len(queryGeneIDList) > 0:
            try:
                KEGGQuery += urllib.request.urlopen(
                    QueryGeneID % "+ncbi-geneid:".join(queryGeneIDList)
                ).read()
            except URLError as e:
                logging.warn(
                    "Could not retrieve GeneID - KEGG result for species:LocusTag query for features %s - %s \n \
                              Error message %s"
                    % (chunkIterator, chunkIterator + i, e.reason)
                )
...
def _get_ECNumberDict(KEGGspeciesLocusTagDict):
    ...
    KeggID_ECDict = {}
    antiSMASHID_ECDict = {}
    keylist = list(KEGGspeciesLocusTagDict.keys())
    ...
    for chunkIterator in range(0, len(keylist), ChunkSizeDataRetrieval):
        logging.debug(
            "ECpredictor/kegg: getting results %s - %s of %s"
            % (chunkIterator, chunkIterator + ChunkSizeDataRetrieval, len(keylist))
        )
        KEGGLocusList = []
        ...
        KEGGQueryResult = urllib.request.urlopen(
            QueryKEGG % "+".join(KEGGLocusList)
        ).read()
        ...
        if sptlist[0] == "///":
            ...
            if len(ECNumbers) > 0:
                if (KEGGLocusTag == "") or (KEGGOrgID == ""):
                    logging.warn(
                        "Error parsing KEGG result; could not parse Locus tag or Organism"
                    )
    ...
    for antiSMASHID, KEGGspeciesLocusTag in list(KEGGspeciesLocusTagDict.items()):
        if KEGGspeciesLocusTag in KeggID_ECDict:
    ...
    for key in list(ECDict.keys()):
        ...
        if len(ECDict[key]) > 0:
            ...
            if "EC_number" in Feature.qualifiers:
                logging.warn(
                    "ECpredictor[kegg]: Overwriting existing EC annotation: %s  with %s"
                    % (
            ...
            logging.warn(
                "ECpredictor[KEGG]: Could not find EC number for %s"
