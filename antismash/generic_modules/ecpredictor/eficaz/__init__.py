...
class EFICAzECPrediction:
    def __init__(self, seq_record, options):
        ...
        self.basedirName = os.path.abspath(
            os.path.join(options.outputfoldername, "EFICAz")
        )
        try:
            ...
            if os.path.exists(self.basedirName):
                ...
                logging.exception(
                    "Cannot create EFICAz output directory %s" % self.basedirName
                )
                sys.exit(1)
    ...
    def _prepareInput(self):
        ...
        for i in range(maxChunks):
            if i == 0:
                ...
                fastaChunk = allFastaList[
                    (i * equalpartsizes) : ((i + 1) * equalpartsizes)
                ]

            # setup separate directories for EFICAz
            chunkDirName = "{basedir}{sep}Chunk{chunk_no:05d}".format(
                basedir=self.tempdirname, sep=os.sep, chunk_no=i + 1
            )
            ...
            chunkFileName = "{dirname}{sep}input_{seqid}_{chunk_no:05d}.fasta".format(
    ...
    def _runEFICAz(self, chunkDir):
        ...
        if not os.path.isfile(os.path.join(self.basedirName, ecpredfile)):
            ...
            logging.debug(
                "executing %s in directory %s" % (" ".join(cmdline), chunkDir)
            )
            try:
            ...
            try:
                shutil.copy(
                    os.path.abspath(os.path.join(self.basedirName, ecpredfile)),
    ...
    def _execute_EFICAz_processes(self, directorynames):
        ...
        for directoryname in directorynames:
            processList.append(
                multiprocessing.Process(target=self._runEFICAz, args=(directoryname,))
            )
        ...
        logging.debug(
            "After joining the processes EC4Dict has %s entries"
            % len(list(self.EC4Dict.keys()))
        )
    ...
    def _parseEFICAzResults(self, chunkDirs):
        for chunkDir in chunkDirs:
            ...
            try:
                ...
                logging.error(
                    "No EFICAz outputfile %s found. Skipping Chunk...\nOSError: %s",
                    ecpredfile,
                    e,
                )
                continue
            ...
            logging.debug(
                "EC4Pred has %s entries for chunk" % len(list(self.EC4Dict.keys()))
            )
    ...
    def _copyFiles(self, chunkDirs):
        ...
        for chunkDir in chunkDirs:
            try:
                ...
                logging.error(
                    "Could not copy eficaz input file %s to destination %s",
                    self.ChunkFilenames[chunkDir],
                    self.basedirName,
                )
            ...
            try:
                ...
                logging.error(
                    "Could not copy eficaz result file %s to destination %s",
                    self.ChunkFilenames[chunkDir] + ".ecpred",
                    self.basedirName,
                )
    ...
    def runECpred(self):
        ...
        if len(chunkDirs) > 0:
            logging.debug(
                "split inputs to %s directories; first one is %s"
                % (len(chunkDirs), chunkDirs[0])
            )
            ...
            logging.warn(
                "ECpredictor: No protein coding sequences found for in record: %s"
                % self.seq_record.id
            )
...
def getECs(seq_record, options):
    ...
    EFICAzECs.runECpred()
    logging.debug(
        "Found %s predictions for EC4" % len(list(EFICAzECs.getEC4Dict().keys()))
    )
    ...
    for feature in utils.get_cds_features(seq_record):
        ...
        if EFICAzECs.getEC3Info(featureID):
            notes.append(
                "EFICAz EC number prediction: EC3: {0}; {1}".format(
                    ", ".join(EFICAzECs.getEC3(featureID)),
