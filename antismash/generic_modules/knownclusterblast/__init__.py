...
from antismash.generic_modules.clusterblast.data_loading import (
    prepare_data,
    generate_Storage_for_cb,
)
...
def check_prereqs(options):
    ...
    for file_name, optional in _required_files:
        if (
            utils.locate_file(path.join(utils.get_full_path(__file__, ""), file_name))
            is None
            and not optional
        ):
