    ...
    if options.gff3:
            ...
            with open(options.gff3) as f:
                for line in f:
                    if line.startswith("#"):
                        ...
                        int(
                            line.split("\t")[3]
                        )  # 4th column has to be a number (start)
        ...
        try:
            ...
            if "CDS" in set(
                [
                    n
                    for key in examiner.parent_child_map(open(options.gff3))
                    for n in key
                ]
            ):
                logging.error(
        ...
        if sub.sub_features:  # If there are sub_features, go deeper
            ...
            if modulus == 0:
                ...
                if sub.strand == 1:
                    TransLocList.append(
                        FeatureLocation(
                            loc[0], loc[1] + (3 - modulus), strand=sub.strand
                        )
                    )
                else:
                    TransLocList.append(
                        FeatureLocation(
                            loc[0] - (3 - modulus), loc[1], strand=sub.strand
                        )
                    )
...
def merge_cds(LocList, TransLocList, QualList, sequence):
    ...
    trans = "".join(
        [n.extract(sequence.seq).translate(stop_symbol="")._data for n in TransLocList]
    )
