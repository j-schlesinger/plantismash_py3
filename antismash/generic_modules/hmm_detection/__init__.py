...
from antismash.generic_modules.signature import Signature
...
class HmmSignature(Signature):
    ...
    def __init__(self, name, description, cutoff, hmm_file):
        ...
        self.name = name
        super(HmmSignature, self).__init__(
            name, "model", description, cutoff, utils.get_full_path(__file__, hmm_file)
        )
...
def get_sig_profiles():
    ...
    for hmm_model in cfg.enabled_detection_models:
        ...
        _signature_profiles.extend(
            [
                HmmSignature(
                    prefix + details[0],
                    details[1],
                    int(details[2]),
                    path.join(dir_path, details[3]),
                )
...
def is_sec_met_feature_of_type(type_, feature):
    ...
    return "sec_met" in feature.qualifiers and feature.qualifiers["sec_met"][
        0
    ].startswith(type_)
...
def find_clusters(seq_record, rulesdict, overlaps):
    ...
    for feature in features:
        within_cutoff = False
        if ("sec_met" in feature.qualifiers) and (
            len([feat for feat in feature.qualifiers["sec_met"] if "Type: " in feat])
            > 0
        ):
            ...
            if len(clusters) > 0:
                ...
                within_gene_num_cutoff = (
                    min(
                        [
                            abs(
                                overlaps[1][utils.get_gene_id(feature)]
                                - overlaps[1][ncds]
                            )
                            for ncds in cluster_cds
                        ]
                    )
                    - 1
                    <= cfg.gene_num_cutoff
            ...
            if not within_cutoff:
                ...
                # Create new cluster
                new_cluster = SeqFeature(
                    FeatureLocation(feature_start, feature_end), type="cluster"
                )
    ...
    if len(clusters) > 0:
        ...
        cluster = clusters[-1]
        cluster.location = FeatureLocation(
            max(0, cluster.location.start - cluster.qualifiers["extension"][0]),
            min(
                len(seq_record),
                cluster.location.end + cluster.qualifiers["extension"][0],
            ),
...
def filter_results(results, results_by_id, overlaps, feature_by_id):
    ...
    for hmm_model in cfg.enabled_detection_models:
        ...
        if hmm_model != "default":
            ...
            prefix = hmm_model + "/"
        for line in (
            open(path.join(dir_path, "filterhmmdetails.txt"), "r").read().split("\n")
        ):
            ...
            for cds in list(results_by_id.keys()):
                ...
                if len(competing_hits) > 1:
                    ...
                    for hit in cdsresults:
                        for otherhit in [
                            cdsresult for cdsresult in cdsresults if hit != cdsresult
                        ]:
                            overlap = len(
                    ...
                    for group in overlapping_groups:
                        ...
                        hit_with_highestscore = group[
                            [hit.bitscore for hit in group].index(highestscore)
                        ]
                        to_delete = [
        ...
        for cds in overlap_id_with_result[overlap_id]:
            for hit in results_by_id[cds]:
                ...
                if (hit.query_id not in best_hit_scores) or (
                    ...
                    best_hit_scores[hit.query_id] = abs(
...
def apply_cluster_rules(
    results_by_id, feature_by_id, enabled_clustertypes, rulesdict, overlaps
):
    "Apply cluster rules to determine if HMMs lead to secondary metabolite core gene detection"
    typedict = {}
    cfg = config.get_config()
    cds_with_hits = sorted(
        list(results_by_id.keys()),
        key=lambda gene_id: feature_by_id[gene_id].location.start,
    )
    for cds in cds_with_hits:
        ...
        for clustertype in [
            ct for ct in enabled_clustertypes if ct not in _type.split("-")
        ]:
            prefix = ""
            ...
            single_rules = [
                rule
                for rule in rulesdict[clustertype][0].split(" or ")
                if " & " not in rule and "cluster(" not in rule
            ]
            combined_rules = [
                rule[1:-1]
                for rule in rulesdict[clustertype][0].split(" or ")
            ...
            if (
                len(set([(prefix + rule) for rule in single_rules]) & set(cdsresults))
                >= 1
            ):
            ...
            for rule in combined_rules:
                ...
                if len(set(required_matches) & set(cdsresults)) == len(
                    required_matches
                ):
                    if not (_type != "none" and clustertype == "other"):
                        if _type == "none" or _type == "other" or _type == clustertype:
            ...
            for rule in cluster_rules:
                ...
                if len(cluster_results) > 0:
                    locations = [
                        feature_by_id[cds].location.start,
                        feature_by_id[cds].location.end,
                    ]
                    for othercds in cds_with_hits:
                        needed_hits_found = list(
                            set([res.query_id for res in results_by_id[othercds]])
                            & set(missing_results)
                        )
                        if len(needed_hits_found) > 0:
                            ...
                            if (
                                min(
                                    [
                                        abs(max(locations) - min(flocations)),
                                        abs(max(locations) - min(flocations)),
                                    ]
                                )
                                < cutoff
            ...
            for rule in minimum_rules:
                ...
                required_matches = [
                    (prefix + rl)
                    for rl in rule.partition("[")[2].partition("]")[0].split(",")
                ]
                essential_matches = [
                    rl for rl in rule.rpartition("[")[2].partition("]")[0].split(",")
                ]
                if essential_matches == [""]:
                    essential_matches = []
                cluster_results = list(set(required_matches) & set(cdsresults))
                missing_essential = list(
                    set(essential_matches)
                    - set(calc_essential_found(essential_matches, cdsresults, prefix))
                )
                ...
                nearest_cds = cds
                if (len(list(set(essential_matches) & set(missing_essential))) > 0) or (
                    nrcds > 0 and nrcds < min_number
                ):
                    # scan neighboring genes forward
                    for othercds in cds_with_hits:
                        if (overlaps[1][othercds] <= overlaps[1][cds]) or (
                            overlaps[1][othercds]
                            in [overlaps[1][ncds] for ncds in neighborcds]
                        ):
                            continue
                        feature = feature_by_id[othercds]
                        flocations = [feature.location.start, feature.location.end]
                        ...
                        if cfg.enable_dynamic_cutoff:
                            ...
                            cutoff = max(
                                int(
                                    base_cutoff
                                    * get_dynamic_cutoff_multiplier(othercds, overlaps)
                                ),
                                int(
                                    base_cutoff
                                    * get_dynamic_cutoff_multiplier(
                                        nearest_cds, overlaps
                                    )
                                ),
                            )
                        within_cutoff = (
                            (
                                min(flocations) >= cluster_start
                                and max(flocations) <= cluster_end
                            )
                            or (
                                cluster_start - cutoff
                                <= max(flocations)
                                <= cluster_start
                            )
                        ...
                        within_gene_num_cutoff = (
                            min(
                                [
                                    abs(overlaps[1][othercds] - overlaps[1][ncds])
                                    for ncds in cluster_cds
                                ]
                            )
                            - 1
                        ...
                        if within_cutoff:
                            othercds_results = [
                                res.query_id for res in results_by_id[othercds]
                            ]
                            # check essential hits
                            essential_found = calc_essential_found(
                                missing_essential, othercds_results, prefix
                            )
                            if len(essential_found) > 0:
                                missing_essential = list(
                                    set(missing_essential) - set(essential_found)
                                )
                            ...
                            # check required hits
                            needed_hits_found = list(
                                set(othercds_results) & set(required_matches)
                            )
                            if len(needed_hits_found) > 0:
                                nrcds += 1
                                # append to neighborcds for in-advance tagging
                ...
                if nrcds >= min_number and len(missing_essential) == 0:
                    if cfg.min_domain_number > 1:
                        ...
                        for ncds in neighborcds:
                            unique_domains = list(
                                set(unique_domains)
                                | set([res.query_id for res in results_by_id[ncds]])
                            )
                        if len(unique_domains) < cfg.min_domain_number:
                            logging.debug(
                                "Cluster dropped due to not passing min. unique domains filtering [%s:%s]"
                                % (
                                    feature_by_id[cds].location.start,
                                    feature_by_id[cds].location.end,
                                )
                    ...
                    if cfg.enable_cdhit:
                        ...
                        if not nrcds >= min_number:
                            logging.debug(
                                "Cluster dropped due to not passing cd-hit filtering [%s:%s]"
                                % (
                                    feature_by_id[cds].location.start,
                                    feature_by_id[cds].location.end,
                                )
...
def detect_signature_genes(seq_record, enabled_clustertypes, options):
    ...
    if options.ignore_short_aa:
        ...
        for f in seq_record.features:
            if (
                f.type == "CDS"
                and len(f.qualifiers["translation"][0]) < min_length_aa
                and utils.get_gene_id(f) not in results_by_id
            ):
    ...
    results, results_by_id = filter_results(
...
def overlaps(feature1, feature2):
    if (
        feature2.location.start <= feature1.location.start <= feature2.location.end
...
def get_dynamic_cutoff_multiplier(gene_id, overlaps, radius=10):
    ...
    while (right_idx - left_idx) <= radius:
        ...
        if right_idx >= (len(overlaps[2]) - 1):
            left_idx -= 1
            continue
        if (overlaps[2][center_idx][0] - overlaps[2][left_idx - 1][1]) > (
            overlaps[2][right_idx + 1][0] - overlaps[2][center_idx][1]
        ):
            right_idx += 1
        else:
            left_idx -= 1

    result = float(overlaps[2][right_idx][1] - overlaps[2][left_idx][0]) / float(
...
def remove_irrelevant_allorfs(seq_record):
    ...
    auto_orf_features = [
        ...
        if "note" in feature.qualifiers and "auto-all-orf" in feature.qualifiers["note"]
    ]
    other_features = [
        feature
        for feature in allfeatures
    ...
    for autofeature in auto_orf_features:
        if "sec_met" not in autofeature.qualifiers:
            ...
            for otherfeature in other_features:
                if (
                    overlaps(autofeature, otherfeature)
                    and "sec_met" in otherfeature.qualifiers
                ):
                    to_delete.append(autofeature)
            ...
            if glimmer_has_sec_met == False:
                for otherfeature in other_features:
                    if (
                        overlaps(autofeature, otherfeature)
                        and "sec_met" not in otherfeature.qualifiers
                    ):
        ...
        for cdhit_cluster in cdhit_table:
            if len(cdhit_cluster["genes"]) > 1:
                cl_features = [
                    feature
                    for feature in features
                    if utils.get_gene_id(feature) in list(cdhit_cluster["genes"].keys())
                ]
                ...
                for cds in cl_features:
                    result = ",".join(
                        [
                            ...
                            for othercds in list(
                                pct_table[utils.get_gene_id(cds)].keys()
                            )
                        ]
                    )
...
def add_additional_nrpspks_genes(typedict, results_by_id, seq_record, nseqdict):
    ...
    othercds_with_results = [
        ...
        if utils.get_gene_id(cds) in results_by_id
    ...
    for cds in othercds_with_results:
        ...
        if len(set(nrpspksdomains) & set(cdsresults)) >= 1:
            _update_sec_met_entry(
                cds, results_by_id[utils.get_gene_id(cds)], "other", nseqdict
            )
...
def _update_sec_met_entry(feature, results, clustertype, nseqdict):
    result = "; ".join(
        [
            "%s (E-value: %s, bitscore: %s, seeds: %s)"
            % (res.query_id, res.evalue, res.bitscore, nseqdict.get(res.query_id, "?"))
            for res in results
        ]
    )
