...
def parse_clusterblast_details(
    ...
    compound_found = "n"
    allcoregenes = [
        utils.get_gene_id(cds) for cds in utils.get_secmet_cds_features(seq_record)
    ]
    if searchtype == "general":
    ...
    for i in details:
        ...
        if hitclusternr <= options.nclusters:
            ...
            for k in blasthitslines:
                ...
                if (
                    k.split("\t")[0] in allcoregenes
                    and k.split("\t")[0] not in coregeneswithhits
                ):
            ...
            if searchtype == "general":
                for k in list(seq_record.known_compound_dict.keys()):
                    if (
                        k in i
                        and compound_found == "n"
                        and len(querygeneswithhits) > 2
                        and len(coregeneswithhits) > 0
                    ):
                        seq_record.closestcompounddict[
                            clusternr
                        ] = seq_record.known_compound_dict[k]
            compound_found = "y"
            # Create Blast dict
            (
                blasthitdict,
                ...
                hitgenes,
                revblasthitdict,
            ) = create_blastdicts(blasthitslines)
            ...
            if j in blasthitdict:
                for k in blasthitdict[j]:
                    ...
                    for l in list(blastdetailsdict.keys()):
                        if (
                            j == l.partition("_|_|_")[0]
                            and k == l.rpartition("_|_|_")[2]
                        ):
...
def create_blastdicts(blasthitslines):
    ...
    for i in blasthitslines:
        ...
        if tabs[0] in blasthitdict:
            ...
            blastdetailsdict[tabs[0] + "_|_|_" + tabs[1]] = [
                tabs[5],
                tabs[3],
                tabs[2],
                tabs[4],
...
def generate_Storage_for_cb(options, seq_record, searchtype="ClusterBlastData"):
    ...
    try:
        clusterBlastResults.internalhomologygroupsdict = (
            seq_record.internalhomologygroupsdict
        )
...
def read_clusterblastfile(seq_record, options, clusternr, searchtype="general"):
    ...
    hitlines = (
        (clusterblastfile.split("Significant hits: \n")[1]).split("\nDetails:")[0]
    ).split("\n")
    a = 0
    cb_accessiondict = {}
    b = 1
    for i in hitlines:
        ...
        if not test_accession(seq_record.id) or (
            ". " + seq_record.id.partition(".")[0] not in i.split("\t")[0]
            and "_" + seq_record.id.partition(".")[0] not in i.split("\t")[0]
        ):
            b += 1
            if a < options.nclusters:
                if len(i) > 3:
                    percentage_genes_with_hits = get_hitnumbers_from_clusterblastfile(
                        clusterblastfile, i
                    )
                if len(i) < 80 and len(i) > 3:
                    toptenhitclusters.append(
                        i
                        + " (%i%% of genes show similarity)"
                        % percentage_genes_with_hits
                    )
                elif len(i) >= 80:
                    j = (
                        i[0:77]
    ...
    if test_accession(seq_record.id):
        details = [
            details
            for details in (clusterblastfile.split("\nDetails:")[1]).split(">>")[1:]
            if (
                seq_record.id.partition(".")[0] not in details.partition("Source:")[0]
                and "_" + seq_record.id.partition(".")[0]
                not in details.partition("Source:")[0]
            )
        ]
    else:
...
def get_hitnumbers_from_clusterblastfile(clusterblastfile, clusterhit):
    ...
    # Get percentage
    blasthits = (
        hitdetails.partition(" %coverage, e-value):\n")[2]
        .partition("\n\n")[0]
        .split("\n")
    )
    nr_genes_with_hits = len(set([hit.split("\t")[1] for hit in blasthits]))
    percentage_genes_with_hits = int(
        (float(nr_genes_with_hits) / float(nr_hitclustergenes)) * 100
    )
...
def load_subclusterblast_outputdata(seq_record, options):
    ...
    for genecluster in geneclusters:
        ...
        (
            ...
            queryclustergenesdetails,
            cb_accessiondict,
        ) = read_clusterblastfile(
            seq_record, options, clusternr, searchtype="subclusters"
        )
        parse_clusterblast_details(
            options,
            seq_record,
            clusternr,
            details,
...
def load_knownclusterblast_outputdata(seq_record, options):
    ...
    for genecluster in geneclusters:
        ...
        (
            ...
            seq_record, options, clusternr, searchtype="knownclusters"
        )
        parse_clusterblast_details(
            options,
            seq_record,
...
def load_clusterblast_outputdata(seq_record, options):
    ...
    for genecluster in geneclusters:
        ...
        (
            ...
            queryclustergenesdetails,
            cb_accessiondict,
        ) = read_clusterblastfile(seq_record, options, clusternr)
        parse_clusterblast_details(
            options,
            seq_record,
            clusternr,
...
def load_genecluster_info(seq_record, options, searchtype="general"):
    ...
    for genecluster in geneclusters:
        ...
        (
            ...
            transporters,
            clustersize,
        ) = retrieve_gene_cluster_annotations(
            seq_record, smcogdict, gtrcoglist, transportercoglist, geneclusternr
        )
        ...
        (
            ...
            krpredictionsdict,
            structpred,
        ) = retrieve_pksnrps_info(seq_record, geneclusternr, pksnrpsprots)
...
def retrieve_gene_cluster_annotations(
    seq_record, smcogdict, gtrcoglist, transportercoglist, geneclusternr
):
    allcoregenes = [
        utils.get_gene_id(cds) for cds in utils.get_secmet_cds_features(seq_record)
    ...
    pksnrpscoregenes = [
        utils.get_gene_id(cds) for cds in utils.get_pksnrps_cds_features(seq_record)
    ]
...
def retrieve_clusterblast_info(seq_record, geneclusternr, searchtype="general"):
    if searchtype == "general":
        hitgeneclusters = list(
            range(1, (seq_record.nrhitgeneclusters[geneclusternr] + 1))
        )
    elif searchtype == "subclusters":
        hitgeneclusters = list(
            range(1, (seq_record.sc_nrhitgeneclusters[geneclusternr] + 1))
        )
    elif searchtype == "knownclusters":
        hitgeneclusters = list(
            range(1, (seq_record.kc_nrhitgeneclusters[geneclusternr] + 1))
        )
    else:
    ...
    for i in pksnrpsprots:
        ...
        for j in doms:
            ...
            if "AMP-binding" in domname or "A-OX" in domname:
                ...
                substrspecminowadict[domname2] = substr_spec_preds.minowa_nrps_preds[
                    i + "_A" + str(nr)
                ]
                substrspecnrpspredictordict[domname2] = [
                    substr_spec_preds.nrps_code_preds[i + "_A" + str(nr)],
                    substr_spec_preds.nrps_svm_preds[i + "_A" + str(nr)],
                ]
                substrspecconsensusdict[domname2] = substr_spec_preds.consensuspreds[
                    i + "_A" + str(nr)
                ]
            if "PKS_AT" in domname:
                domname2 = i + "_" + "AT" + str(nr)
                substrspecminowadict[domname2] = substr_spec_preds.minowa_pks_preds[
                    i + "_AT" + str(nr)
                ]
                substrspecpkssigdict[domname2] = substr_spec_preds.pks_code_preds[
                    i + "_AT" + str(nr)
                ]
                substrspecconsensusdict[domname2] = substr_spec_preds.consensuspreds[
                    i + "_AT" + str(nr)
            ...
            if "CAL_domain" in domname:
                ...
                substrspecminowadict[domname2] = substr_spec_preds.minowa_cal_preds[
                    i + "_CAL" + str(nr)
                ]
                substrspecconsensusdict[domname2] = substr_spec_preds.consensuspreds[
                    i + "_CAL" + str(nr)
            ...
            if "PKS_KR" in domname:
                ...
                krpredictionsdict[domname2] = [
                    substr_spec_preds.kr_activity_preds[i + "_KR" + str(nr)],
                    substr_spec_preds.kr_stereo_preds[i + "_KR" + str(nr)],
                ]
        pksnrpsdomains[i] = [domlist, domsdetails]
    structpred = utils.get_structure_pred(
        utils.get_cluster_by_nr(seq_record, geneclusternr)
    )
    return (
        pksnrpsprotsnames,
