...
def alignsmcogs(smcog, inputnr):
    ...
    if sys.platform == ("linux2") or sys.platform == ("win32"):
        musclecommand = [
            ...
            infile1,
            "-in2",
            "input" + str(inputnr) + ".fasta",
            "-out",
            "muscle" + str(inputnr) + ".fasta",
        ]
...
def drawtree(inputnr):
    ...
    fasttreecommand = (
        "fasttree -quiet -fastest -noml trimmed_alignment"
        + str(inputnr)
        + ".fasta > "
        + nwkfile
    )
...
def converttree(inputnr, smcogsoutputfolder, tag):
    ...
    command = [
        ...
        "-jar",
        utils.get_full_path(__file__, "TreeGraph.jar"),
        "-convert",
        "tree%s.nwk" % inputnr,
        "-xtg",
        "tree%s.xtg" % inputnr,
