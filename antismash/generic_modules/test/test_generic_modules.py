...
class GenericModulesTest(unittest2.TestCase):
    def setUp(self):
        ...
        mock(
            "antismash.generic_modules.fullhmmer.check_prereqs",
            returns=["fullhmmer"],
            tracker=self.tt,
        )
        mock(
            "antismash.generic_modules.genefinding.check_prereqs",
            returns=["genefinding"],
            tracker=self.tt,
        )
        mock(
            "antismash.generic_modules.hmm_detection.check_prereqs",
            returns=["hmm_detection"],
            tracker=self.tt,
        )
        mock(
            "antismash.generic_modules.clusterblast.check_prereqs",
            returns=["clusterblast"],
            tracker=self.tt,
        )
        mock(
            "antismash.generic_modules.subclusterblast.check_prereqs",
            returns=["subclusterblast"],
            tracker=self.tt,
        )
        mock(
            "antismash.generic_modules.knownclusterblast.check_prereqs",
            returns=["knownclusterblast"],
            tracker=self.tt,
        )
        mock(
            "antismash.generic_modules.smcogs.check_prereqs",
            returns=["smcogs"],
            tracker=self.tt,
        )
