...
class active_site_finder(object):
    ...
    def __init__(self, seq_record, options):
        ...
        if "activeSiteFinderConf" not in options:
            options.activeSiteFinderConf = path.join(
                utils.get_full_path(__file__, ""), "config", "SignatureResources.xml"
            )

        if "activeSiteFinderHMMDir" not in options:
            options.activeSiteFinderHMMDir = path.join(
                utils.get_full_path(__file__, ""), "hmm"
            )
        ...
        try:
            ...
            logging.exception(
                "Could not load/parse ActiveSiteFinder configuration file %s.",
                options.activeSiteFinderConf,
            )
            sys.exit(1)
    ...
    def execute(self):
        ...
        for analysisResource in analysisResources:
            ...
            primaryTagType = analysisResource.find(
                "./Prerequisite/primary_tag_type"
            ).text
            ...
            SeqFeatureList = utils.get_all_features_of_type_with_query(
                seq_record, primaryTagType, tag, tagValue
            )
            ...
            for result in results:
                ...
                if len(result.hsps) > 0:
                    if result.hsps[0].aln[0].id != result.id:
                        ...
                        logging.exception(
                            "Was looking for hit %s but got hit for %s instead",
                            domain_id,
                            result.id,
                        )
                        break
                    ...
                    if ASF_string:
                        ...
                        if (
                            self.options.QualifierTags.asf_scaffold
                            not in SeqFeature.qualifiers
                        ):
                    ...
                    (
                        ASF_stringList,
                        choiceResultList,
                    ) = self._get_prediction_annotation(result, predictionChoicesXML)

                    for ASF_string in ASF_stringList:
                        if (
                            self.options.QualifierTags.asf_choice
                            not in SeqFeature.qualifiers
                        ):
                            SeqFeature.qualifiers[
                                self.options.QualifierTags.asf_choice
                        ...
                        logging.debug(
                            "adding ASF choice info to %s %s..%s:",
                            SeqFeature.type,
                    ...
                    for choiceResult in choiceResultList:
                        if (
                            self.options.QualifierTags.asf_prediction
                            not in SeqFeature.qualifiers
                        ):
                            SeqFeature.qualifiers[
                                self.options.QualifierTags.asf_prediction
                        ...
                        logging.debug(
                            "adding ASF choiceResult info to %s %s..%s:",
                            SeqFeature.type,
                            SeqFeature.location.start,
                            SeqFeature.location.end,
                        )
                        ...
                        correspondingCDSFeatures = (
                            utils.get_all_features_of_type_with_query(
                                seq_record,
                                "CDS",
                                "locus_tag",
                        ...
                        if not len(correspondingCDSFeatures) == 1:
                            logging.warning(
                                "ASF: found %s entries for CDS with locus tag %s, skipping",
                                len(correspondingCDSFeatures),
                                utils.get_gene_id(SeqFeature),
                            )
                            continue
                        ...
                        if SeqFeature.strand == 1:
                            start = (
                                (
                                    SeqFeature.location.start
                                    - correspondingCDSFeature.location.start
                                    + 3
                                )
                                / 3
                            ) - 1
                            ...
                            start = (
                                (
                                    correspondingCDSFeature.location.end
                                    - SeqFeature.location.end
                                    + 3
                                )
                                / 3
                        ...
                        sec_met_string += (
                            SeqFeature.qualifiers["domain"][0]
                            + " ("
                            + str(start)
                            + ".."
                            + str(end)
                        ...
                        logging.debug(
                            "adding ASF-prediction data to sec_met qualifier of %s",
                            utils.get_gene_id(correspondingCDSFeature),
                        )
                        correspondingCDSFeature.qualifiers["sec_met"].append(
                            sec_met_string
                        )
                    if has_annotation:
                        if (
                            self.options.QualifierTags.asf_note
                            not in SeqFeature.qualifiers
                        ):
                            SeqFeature.qualifiers[
                                self.options.QualifierTags.asf_note
                        ...
                        SeqFeature.qualifiers[
                            ...
                            "ASF analyisis with definition %s (type %s)"
    ...
    def check_prereqs(self):
        ...
        for HMMProfile in self.HmmProfilesFilenameObj:
            if (
                utils.locate_file(
                    path.join(options.activeSiteFinderHMMDir, HMMProfile.text)
                )
                is None
            ):
    ...
    def _get_scaffold_annotation(self, result, scaffoldXML):
        ...
        for i in range(0, len(hmm_seq)):
            ...
            if str(offset) in scaffoldPosList:
                ...
                matchLineStr.append(" ")
        logging.debug(
            "%s %s..%s", query_seq, result.hsps[0].query_start, result.hsps[0].query_end
        )
        logging.debug(
            "%s %s..%s", hmm_seq, result.hsps[0].hit_start, result.hsps[0].hit_end
        )
        ...
        for i in range(0, len(scaffoldPosList)):
            ...
            if (result.hsps[0].hit_start > scafPos) or (
                result.hsps[0].hit_end < scafPos
            ):
            ...
            try:
                fixedScafPos = self._fix_coordinates(
                    scafPos - result.hsps[0].hit_start, hmm_seq
                )
            except ValueError:
                logging.error(
                    "gap-fixed scaffold coordinate %s outside hsp for original position: ",
                    scafPos,
                )
                overallMatch = False
                skip = True
            ...
            try:
                if hmm_seq[fixedScafPos].lower() != scafValue.lower():
                    logging.warn(
                        "ASF: aa extracted from hmm profile does not match predifined aa in XML config file!"
                    )
            except IndexError:
                logging.warn(
                    "gap-fixed scaffold coordinate %s outside hsp!", fixedScafPos
                )
                overallMatch = False
                skip = True
                break
            ...
            logging.debug(
                "Scaffold coordinate %s; fixed scaffold coordinate %s, query aa %s; hmm aa %s; "
                "scaffold value %s; emission probability %s; match %s",
                scafPos,
                fixedScafPos,
                extracted_aa,
                hmm_seq[fixedScafPos],
                scafValue,
                extracted_aa_Emission,
                match,
            )
        ...
        if not skip:
            ASF_string = (
                "Scaffold coordinates: (%s); scaffold residues: (%s); expected: (%s); matchArray: (%s); "
                "emission probability array (%s); overall match: %s"
                % (
                    ",".join(scaffoldPosList),
                    ",".join(extracted_aa_List),
                    ",".join(scaffoldValueList),
                    ",".join(match_List),
                    ",".join(emission_List),
                    str(overallMatch).upper(),
                )
    ...
    def _get_prediction_annotation(self, result, predictionChoicesXML):
        ...
        for choice in predictionChoicesXML:
            ...
            for i in range(0, len(predictionOffsetList)):
                ...
                if (result.hsps[0].hit_start > predictionOffset) or (
                    result.hsps[0].hit_end < predictionOffset
                ):
                    logging.warn(
                        "choice/prediction coordinate %s outside hsp!", predictionOffset
                    )
                ...
                try:
                    fixed_predictionOffset = self._fix_coordinates(
                        predictionOffset - result.hsps[0].hit_start, hmm_seq
                    )
                except ValueError:
                    logging.warn(
                        "gap-fixed choice/prediction coordinate %s outside hsp! for result: %s, which has length: %s",
                        predictionOffset,
                        result.id,
                        len(query_seq),
                    )
                ...
                try:
                    ...
                    logging.error(
                        "gap-fixed choice/prediction coordinate %s outside hsp! for result: %s, which has length: %s; this should never happen",
                        fixed_predictionOffset,
                        result.id,
                        len(query_seq),
                    )
                ...
                logging.debug(
                    "Offset %s; fixed offset %s  Expected: %s; observed in query %s: observed in hmm %s; Emission %s; match %s",
                    predictionOffset,
                    fixed_predictionOffset,
                    predictionValue,
                    extracted_aa,
            ...
            logging.debug(
                "Overall Match for prediction %s: %s",
                choice.attrib["result"],
            ...
            if not skip:
                ASF_string = (
                    "Description: %s, choice result: %s, choice coordinates: (%s); residues: (%s); "
    ...
    def _execute_tool(self, analysisResource, fileName=None, stdin_data=None):
        ...
        for parameter in list(
            analysisResource.findall("./Execute/parameters/parameter")
        ):
        ...
        cmdlineList.append(
            utils.locate_file(
                path.join(self.options.activeSiteFinderHMMDir, database.text)
            )
        )
        ...
        try:
            ...
            logging.warn(
                "Error parsing results for active site finder analysis: %s ; no hits will be reported",
                e,
            )
            results = []
    ...
    def _fix_coordinates(self, coordinate, seq):
        ...
        if len(seq) - seq.count(".") - 1 < coordinate:
            logging.error(
                "tried to fix coordinate failed; coordinate exceeds sequence length. This should never happen!"
            )
