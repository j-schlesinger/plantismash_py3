...
def main():
    ...
    group = parser.add_argument_group(
        "CoExpress specific options", "", param=["--coexpress"]
    )
    group.add_argument(
        "--coexpress-soft_file",
    ...
    group = parser.add_argument_group(
        "ClusterFinder specific options", "", param=["--inclusive", "--borderpredict"]
    )
    group.add_argument(
        "--cf_cdsnr",
        dest="cdsnr",
    ...
    options.sequences = [
        filename.replace("< > HYPHEN < >", "-") for filename in options.sequences
    ]
    ...
    logging.info(
        "starting antiSMASH {version}, called with {cmdline}".format(
            version=utils.get_version(), cmdline=" ".join(sys.argv)
        )
    )
    ...
    if options.nclusters > 50:
        logging.info(
            "Number of clusters (%d) is too large. Reducing to 50.", options.nclusters
        )
    ...
    if options.input_type == "prot" and (
        options.clusterblast
        or options.knownclusterblast
        or options.subclusterblast
        or options.inclusive
        or options.full_hmmer
    ):
        logging.error(
            "Protein input option is not compatible with --clusterblast, --subclusterblast, "
    ...
    if options.coexpress:
        ...
        if options.coexpress_soft_file == "" and options.coexpress_csv_file == "":
            logging.error(
                "Required .soft or .csv not found (set with --coexpress-soft_file PATH or --coexpress-csv_file PATH)."
            )
            sys.exit(2)
    ...
    if options.debug and os.path.exists(options.dbgclusterblast):
        logging.debug(
            "Using %s instead of computing Clusterblasts and variantes!",
            options.dbgclusterblast,
        )

    if (
        "train_%s" % options.glimmerhmm_train_folder
    ) not in genefinding.get_supported_training_models():
        logging.error(
            "Specified glimmerHMM training models not found (%s)!",
            options.glimmerhmm_train_folder,
        )
        sys.exit(1)
    ...
    if len(seq_records) < 1:
        logging.error(
            "Sequence file is incorrectly formatted or contains no sequences of sufficient quality."
        )
        sys.exit(1)
    ...
    if options.coexpress:
        ...
        for geo in options.geo_dataset:
            ...
            if i > 0:
                logging.warning(
                    "Duplicated GEO rec_id found, renaming to %s..." % geo_id
                )
                geo["info"]["id"] = geo_id
        ...
        for i in range(0, len(options.geo_dataset)):
            logging.debug(
                ...
                % (i + 1, len(options.geo_dataset))
            )
            for seq_record in seq_records:
                logging.debug("Seq_record %s.." % (seq_record.id))
                options.gene_expressions[i][
                    seq_record.id
                ] = coexpress.match_exp_to_genes(
                    features_to_match[seq_record.id], options.geo_dataset[i]
                )
                ...
                for overlap in overlaps:
                    ...
                    for j in range(0, len(overlap)):
                        ...
                        if gene_id in all_gene_expressions:
                            ...
                            exps = [
                                all_gene_expressions[gene_id]["exp"][sample]
                                for sample in all_gene_expressions[gene_id]["exp"]
                            ]
    ...
    for seq_record in seq_records:
            ...
            if options.coexpress:
                for i in range(0, len(options.gene_expressions)):
                    if old_seq_id in options.gene_expressions[i]:
                        options.gene_expressions[i][
                            seq_record.id
                        ] = options.gene_expressions[i][old_seq_id]
        ...
        if options.use_db and not options.db_overwrite:
            if utils.check_if_dbrecord_exists(seq_record.name, options):
                logging.warn(
                    "A record with accession number %s exits in antiSMASH-DB; "
                    "using this record instead of re-calculating everything\n"
                    "If a recalculation is desired, please add --dboverwrite parameter to command line",
                    seq_record.name,
                )
                try:
                    ...
                    logging.exception(
                        "Uncaptured error %s when reading entries from antiSMASH-DB. This should not have happened :-(",
                        e,
                    )
                    sys.exit(1)
                ...
                if (
                    "SubClusterBlastData"
                    in options.extrarecord[seq_record.id].extradata
                ):
                ...
                if (
                    ...
                    options.knownclusterblast = None
                if (
                    "MetabolicModelDataObj"
                    in options.extrarecord[seq_record.id].extradata
                ):
                    options.modeling = "db"
                    options.metabolicmodeldir = (
                        options.outputfoldername + os.sep + "metabolicModel"
                    )
        ...
        options.orig_record_idx += 1
        logging.debug(
            "The record %s is originating from db %s",
            seq_record.name,
            options.from_database,
        )

    # update coexpress data to calculate all possible high corelation betwen genes in clusters
    if options.coexpress:
        for i in range(0, len(options.geo_dataset)):
            coexpress.update_dist_between_clusters(
                seq_records, options.gene_expressions[i], options.geo_dataset[i]
            )
    ...
    if options.coexpress:
        ...
        for geo_id in utils.get_all_geo_rec_ids(seq_records):
            options.coexpress_inter_clusters[
                geo_id
            ] = coexpress.get_inter_cluster_relation(seq_records, geo_id)
            ...
            if "sec_met" in cds.qualifiers:
                ...
                for row in cds.qualifiers["sec_met"]:
                    if row.startswith("Type: "):
                        ...
                        temp_qual.append(
                            "Domains detected: " + ";".join(cluster_results)
                        )
                    else:
    ...
    logging.debug(
        "antiSMASH calculation finished at %s; runtime: %s",
        str(end_time),
        str(running_time),
    )
...
def run_analyses(seq_record, options, plugins):
    ...
    if len(utils.get_cluster_features(seq_record)) > 0:
        ...
        if options.clusterblast:
            utils.log_status(
                "ClusterBlast analysis for contig #%d" % options.record_idx
            )
        ...
        if options.subclusterblast:
            utils.log_status(
                "SubclusterBlast analysis for contig #%d" % options.record_idx
            )
        ...
        if options.knownclusterblast:
            utils.log_status(
                "KnownclusterBlast analysis for contig #%d" % options.record_idx
            )
        ...
        if options.coexpress:
            utils.log_status(
                "Coexpression analysis for contig #%d" % options.record_idx
            )
            for i in range(0, len(options.geo_dataset)):
                coexpress.run_coexpress(
                    seq_record, options.gene_expressions[i], options.geo_dataset[i]
                )
...
def filter_plugins(plugins, options, clustertypes):
    if (
        options.enabled_cluster_types is None
        or options.enabled_cluster_types == clustertypes
    ):
        return

    for plugin in plugins:
        if (
            plugin.name in clustertypes
            and plugin.name not in options.enabled_cluster_types
        ):
...
def setup_logging(options):
    ...
    if "logfile" in options:
        if not (
            ...
            or os.path.exists(path.dirname(options.logfile))
        ):
    ...
    for contigrange in contigranges:
        ...
        for char in startnumber:
            if char == "0":
                ...
                break
        contigrange = [
            alpha_tag + nrzeros * "0" + str(number)
            for number in range(int(startnumber), int(endnumber))
        ]
        allcontigs.extend(contigrange)
    # Create contig groups of 50 (reasonable download size per download)
    nr_groups = len(allcontigs) / 50 + 1
    contig_groups = [
        list(np_array) for np_array in array_split(array(allcontigs), nr_groups)
    ]
    # Return unchanged if no contigs supplied
    if len(contig_groups[0]) == 0 or (
        len(contig_groups[0]) == 1 and contig_groups[0][0] == ""
    ):
        return [seq_record]
    ...
    for part in contig_info.split(","):
        if "gap(" not in part:
            accessions.append(
                part.partition(":")[0]
                .partition(".")[0]
                .rpartition("complement(")[2]
                .replace(")", "")
            )
    # Create contig groups of 50 (reasonable download size per download)
    nr_groups = len(accessions) / 50 + 1
    contig_groups = [
        list(np_array) for np_array in array_split(array(accessions), nr_groups)
    ]
    # Return unchanged if no contigs supplied
    if len(contig_groups[0]) == 0 or (
        len(contig_groups[0]) == 1 and contig_groups[0][0] == ""
    ...
    for part in allcontigparts:
        if "gap(" in part:
            ...
            if candidate_gap_int.isdigit():
                ...
                logging.error(
                    "Parsing supercontig file failed: faulty gap identifier" + part
                )
                sys.exit(1)
        else:
            accession = (
...
def add_translations(seq_records):
    ...
    for seq_record in seq_records:
        ...
        for cdsfeature in cdsfeatures:
            if (
                "translation" not in cdsfeature.qualifiers
                or len(cdsfeature.qualifiers["translation"]) == 0
            ):
                if len(seq_record.seq) == 0:
                    logging.error(
                        "No amino acid sequence in input entry for CDS %r, "
                    ...
                    try:
                        translation = str(
                            utils.get_aa_translation(seq_record, cdsfeature)
                        )
                    except Bio.Data.CodonTable.TranslationError as e:
                        logging.error(
                            "Getting amino acid sequences from %s, CDS %r failed: %s",
                            seq_record.name,
                            cdsfeature.id,
                            e,
                        )
...
def fix_id_lines(options, filename):
    ...
    if filetype == "fasta":
        ...
        if "LOCUS       " not in file_content.partition("\n")[0]:
            file_content = (
                "LOCUS       A01                    0 bp        DNA              BCT 01-JAN-2000\n"
                + file_content
            )
            needed_fixing = True
    elif filetype == "embl":
        if "ID   " not in file_content.partition("\n")[0]:
            file_content = (
                "ID   A01; SV 1; linear; unassigned DNA; STD; PRO; 0 BP.\nXX\n"
                + file_content
            )
            needed_fixing = True
    ...
    if needed_fixing:
        ...
        new_name = path.join(
            options.full_outputfolder_path, "{}_fixed{}".format(name, ext)
        )
        filename = new_name
...
def parse_input_sequences(options):
    ...
    for filename in filenames:
        ...
        infile.close()
        if (
            "Resource temporarily unavailable" in file_content[:200]
            or "<h1>Server Error</h1>" in file_content[:500]
            or "NCBI - WWW Error" in file_content[:500]
        ):
            logging.error(
                "ERROR: NCBI server temporarily unavailable: downloading %s failed.",
    ...
    for sequence in sequences:
        ...
        cdsfeatures_with_translations = [
            cdsfeature
            for cdsfeature in cdsfeatures
            if "translation" in cdsfeature.qualifiers
        ]
        if len(sequence.seq) == 0 or (
            options.input_type == "nucl"
            and len(str(sequence.seq).replace("N", "")) == 0
            and len(cdsfeatures_with_translations) < 0.8 * len(cdsfeatures)
        ):
        ...
        if options.input_type == "prot":
            if is_nucl_seq(sequence.seq):
                logging.error(
                    "Record %s is a nucleotide record, skipping.", sequence.id
                )
                continue
    ...
    if options.limit > -1:
        ...
        sequences = sorted(
            sequences, key=lambda sequence: len(sequence.seq), reverse=True
        )
    ...
    if new_len < old_len:
        ...
        logging.warning(
            "Only analysing the first %d records (increase via --limit)" % options.limit
        )
    ...
    for sequence in sequences:
        ...
        if len(utils.get_cds_features(sequence)) < 1:
            if options.gff3:
                if (
                    sequence.id in options.gff_ids
            ...
            concated_seq_loc[-1] = (
                s_pos,
                e_pos,
            )  # take note of the contig's start and end position
            num_concated += 1
    if num_concated > 0:
        concat_seq = SeqRecord(Seq(concat_seq_text, seq_type))
        logging.info(
            "Running genefinding on %s Contigs without annotations.." % num_concated
        )
        genefinding.find_genes(concat_seq, options)
        for det_gene in utils.get_cds_features(
            concat_seq
        ):  # pass the annotations back into the sequences
            ...
            for ci in range(0, len(concated_seq_loc)):
                ...
                if (e_pos >= gene_start >= s_pos) and (e_pos >= gene_end >= s_pos):
                    det_gene.location = FeatureLocation(
                        (gene_start - s_pos), (e_pos - gene_end)
                    )
    ...
    new_seqs = []
    for sequence in sequences:
        if len(utils.get_cds_features(sequence)) < 1 and (
            (not options.gff3) or (sequence.id not in options.gff_ids)
        ):
            continue
        new_seqs.append(sequence)
    if len(sequences) > len(new_seqs):
        logging.info(
            "Discarding %s contigs without annotations after gene finding.."
            % (len(sequences) - len(new_seqs))
        )
    sequences = new_seqs
    ...
    for sequence in sequences:
        ...
        if len(utils.get_cds_features(sequence)) < 1:
            if options.gff3:
                logging.info(
                    "No CDS features found in record %r but GFF3 file provided, running GFF parser.",
                    sequence.id,
                )
    ...
    for sequence in sequences:
        ...
        if seq_id not in ids_used:
            ...
            options.all_record_ids.append(
                sequence.id
            )  # Update all_record_ids with new record
    ...
    _signature_profiles = [
        (sig.name, sig.description, sig.cutoff, sig.path)
        for sig in hmm_detection.get_sig_profiles()
    ]
    full_fasta = ""
    results_by_id = {}
    ...
    for seq_record in sequences:
        ...
        for feature in farray:
            ...
            if cur_count < schunk_size:
                ...
                logging.debug(
                    "Running hmmsearch for %s sequences X %s models (%s/%s)"
                    % (cur_count, len(_signature_profiles), cur_total, total_count)
                )
                for sig in _signature_profiles:
                    runresults = utils.run_hmmsearch(
                        utils.get_full_path(
                            __file__,
                            path.join(
                                "antismash", "generic_modules", "hmm_detection", sig[3]
                            ),
                        ),
                        full_fasta,
                        sig[2],
    ...
    del contigs_hits
    logging.info(
        "Removing %i out of %i contigs without any pHMM hits"
        % (len(sequences) - len(new_sequences), len(sequences))
    )
    ...
    if len(sequences) > 1:
        ...
        options.end = -1
        logging.info(
            "Discarding --from and --to options, as multiple entries are used."
        )
    ...
    while i < len(sequences):
        ...
        if options.start > 1:
            if options.start > len(sequence):
                logging.error(
                    "Specified analysis start point is at %r, which is larger "
                    "than record size %r",
                    options.start,
                    len(sequence),
                )
        ...
        if options.end > 0:
            if options.end > len(sequence):
                logging.error(
                    "Specified analysis end point is at %r, which is larger "
                    "than record size %r",
                    options.end,
                    len(sequence),
                )
        ...
        if sequence.seq.find("-") > -1:
            sequence.seq = Seq(
                str(sequence.seq).replace("-", "N"), alphabet=sequence.seq.alphabet
            )

        # Some programs like to write gaps as X, translate() hates that
        if sequence.seq.find("X") > -1:
            sequence.seq = Seq(
                str(sequence.seq).replace("X", "N"), alphabet=sequence.seq.alphabet
            )
        if sequence.seq.find("x") > -1:
            sequence.seq = Seq(
                str(sequence.seq).replace("x", "N"), alphabet=sequence.seq.alphabet
...
def detect_geneclusters(seq_record, options):
    if options.input_type == "nucl":
        utils.log_status(
            "Detecting secondary metabolite signature genes "
    ...
    if options.inclusive:
        utils.log_status(
            "Detecting secondary metabolite clusters using "
...
def cluster_specific_analysis(plugins, seq_record, options):
    ...
    for plugin in plugins:
        ...
        if "specific_analysis" in dir(plugin):
            logging.debug(
                "Running analyses specific to %s clusters", plugin.short_description
            )
            plugin.specific_analysis(seq_record, options)
        else:
            logging.debug(
                "No specific analyses implemented for %s clusters",
...
def unspecific_analysis(seq_record, options):
    ...
    if options.full_hmmer:
        utils.log_status(
            "Running full-genome PFAM analysis for contig #%d" % options.record_idx
        )
        fullhmmer.run(seq_record, options)
...
def check_signature_gene_presence(seq_records, options):
    ...
    logging.info(
        "Checking gene clusters for presence of profile hits using HMM library"
    )
