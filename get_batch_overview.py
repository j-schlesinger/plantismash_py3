    ...
    for fpath in files:
        ...
        for seq_record in seq_records:
                ...
                if "sec_met" in cds.qualifiers:
                    ...
                    for row in cds.qualifiers["sec_met"]:
                        if row.startswith("Type: "):
                            ...
                            cluster_results = []
                            for cluster_result in row.split("Domains detected: ")[
                                -1
                            ].split(";"):
            ...
            for cluster in utils.get_cluster_features(seq_record):
                ...
                if (
                    cluster_type
                    not in res_object[fpath][seq_record.id]["cluster_types"]
                ):
                ...
                if (
                    num_domain
                    > res_object[fpath][seq_record.id]["largest_domain_variations"]
                ):
                    res_object[fpath][seq_record.id][
                        "largest_domain_variations"
                    ] = num_domain
            if len(cdhit_numbers) > 0:
                res_object[fpath][seq_record.id]["average_cdhit"] = numpy.median(
                    cdhit_numbers
                )
            if len(domain_numbers) > 0:
                res_object[fpath][seq_record.id][
                    "average_domain_variations"
                ] = numpy.median(domain_numbers)
