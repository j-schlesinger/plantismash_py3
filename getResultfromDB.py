...
def main():
    ...
    group.add_argument(
        "--dbnamespace", dest="dbnamespace", help="Define BioSQL namespace to search"
    )
    ...
    logging.debug(
        "Number of clusters to show in clusterblast = " + str(options.nclusters)
    )
    if options.seed != 0:
    ...
    for seq_record in seq_records:
        ...
        logging.debug(
            "Keys of extradataHash: %s" % ", ".join(list(extradataHash.keys()))
        )
